...
public:
	...
	friend UINT Thread_AbstractCameraGrab(void* pParam);
	...
	BOOL find_panel_mark_edge_line(BYTE* pImage, int nJob, int nCam, int nPos, double *PosX, double *PosY, CViewerEx* mViewer = NULL);
	...
	void init_algorithm();
	void initPLC_Time();
	void InitBaslerCameras();
	void InitDahuaCameras();
	...
	BOOL NozzleTipSizeCompare(int nJob);	//SJB 2022-12-19 Nozzle Tip Size 판정 추가
};
#endif
