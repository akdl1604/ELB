...
public:
	...
	BOOL calcPanelAvgBrightness(cv::Mat &pSrcImage, int nPanel, CRect rectROI);
	BOOL panelValidPosCheck(cv::Mat& pSrcImage);

	int searchUTG(cv::Mat& pSrcImage, CRect rectROI);
	int searchUTG2(cv::Mat& pSrcImage, CRect rectROI);
	int searchSeparator(cv::Mat& pSrcImage, CRect rectROI);
	int searchSeparator2(BYTE*pImage, CRect rectROI, int nThresh);

	void inspectionPanelExist(cv::Mat& pSrcImage);
