...
enum CALIPER_SEARCH_TYPE {
	...
	SEARCH_TYPE_PEAK,
	SEARCH_TYPE_POS,
	SEARCH_TYPE_POS_SECOND,
	SEARCH_TYPE_POS_THIRD,
	SEARCH_TYPE_PEAK_POS,
};

enum CALIPER_SEARCH_DIR {
	SEARCH_DIR_DARK_TO_LIGHT = 0,
	SEARCH_DIR_LIGHT_TO_DARK,
	SEARCH_DIR_BOTH,
};
...
private:
	...
	cv::Point2f***m_ptCaliper_LUT = NULL;
	cv::Point2f***m_ptCaliperCircle_LUT = NULL;
...
public:
	...
	void processCaliper(cv::Mat *pImg);
	...
	void makeLineFromCaliperInfo(cv::Mat*pImg);
	...
	void DrawCaliper(cv::Mat *pImg);
	void DrawCaliper(FakeDC*pDC);
	void DrawCaliper_Circle(cv::Mat *pImg);
