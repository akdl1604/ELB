...
void CCaliper::DrawCaliper_Circle(cv::Mat *pImg)
{
	// Draw Base Line
	...
	cv::line( *pImg, cv::Point( pt1.x, pt1.y ), cv::Point(pt2.x, pt2.y), cv::Scalar(255, 0, 255));
	cv::circle( *pImg, cv::Point( pt1.x, pt1.y ), 3, cv::Scalar(255, 255, 0));
	cv::circle( *pImg, cv::Point(pt2.x, pt2.y), 3, cv::Scalar(255, 255, 0));
	...
	for(int i = 0; i < m_nNumOfCalipers; i++)
	{
		...
		for(int n = 0; n < 4; n++)
		{
			...
			if( m_AreaInfo[i].use != TRUE  )
				cv::line(*pImg, pt1, pt2, cv::Scalar(128, 128, 255));
			else
			{
				if( i == 0 )	cv::line(*pImg, pt1, pt2, cv::Scalar(128, 255, 128));
				else			cv::line(*pImg, pt1, pt2, cv::Scalar(255, 128, 128));
			}
		}
		...
		cv::putText(*pImg, std::string(strTemp), pt, CV_FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 255));
	}
	...
	if( m_bDirectionSwap )
	{
		...
		cv::line(*pImg, pt1, pt2, cv::Scalar(255, 128, 255));
		...
		cv::line(*pImg, pt1, pt4, cv::Scalar(255, 128, 255));
	...
	else
	{
		...
		cv::line(*pImg, pt2, pt4, cv::Scalar(128, 255, 255), 1);
		...
		cv::line(*pImg, pt2, pt4, cv::Scalar(255, 128, 255));
...
void CCaliper::makeLineFromCaliperInfo(cv::Mat *pImg)
{
	int nSize = 0;
	...
	if( nSize > 1 )
	{
		...
		if( aspectratio < 1.0 )
		{
			cv::Point ptLineInfo[2];

			ptLineInfo[0].y = (int)(lineInfo.a * 0 + lineInfo.b);
			ptLineInfo[0].x = 0;

			ptLineInfo[1].y = (int)(lineInfo.a * pImg->cols + lineInfo.b);
			ptLineInfo[1].x = pImg->cols;
			
			cv::line( *pImg, ptLineInfo[0], ptLineInfo[1], cv::Scalar(255, 255, 255), 3);
		}
		else
		{
			cv::Point ptLineInfo[2];
			...
			ptLineInfo[1].x = (int)((pImg->rows - lineInfo.b) / lineInfo.a);
			ptLineInfo[1].y = pImg->rows;
...
void CCaliper::processCaliper(cv::Mat *pImg)
{
	...
	searchCandidateEachCaliper((BYTE*)pImg->data, pImg->cols, pImg->rows);
	...
	*/
	...
	for(int i = 0; i < m_nNumOfCalipers; i++)
	{
		...
		{
			cv::circle(*pImg, cv::Point((int)m_FindInfo[i].x, (int)m_FindInfo[i].y), 5, cv::Scalar(64, 64, 64), 2);
	...
	for(int i = 0; i < m_nNumOfCalipers; i++)
	{
		...
		if( m_FindInfo[i].ignore )
		{
			...
			for(int n = 0; n < 4; n++)
			{
				...
				cv::line(*pImg, pt1, pt2, cv::Scalar(128, 128, 128), 1);
			}
		}
	}
...
void CCaliper::DrawCaliper(cv::Mat *pImg)
{
	// Draw Base Line 
	cv::Point pt1, pt2;
	...
	cv::line( *pImg, pt1, pt2, cv::Scalar(128, 128, 128) );		// Base Line

	int nOffset = 3;
	
	cv::line( *pImg, cv::Point( pt1.x, pt1.y ), cv::Point(pt2.x, pt2.y), cv::Scalar(255, 0, 255));
	cv::circle( *pImg, cv::Point( pt1.x, pt1.y ), 3, cv::Scalar(0, 255, 255));
	cv::circle( *pImg, cv::Point(pt2.x, pt2.y), 3, cv::Scalar(0, 255, 255));
	...
	for(int i = 0; i < m_nNumOfCalipers; i++)			// Caliper
	{
		...
		for(int n = 0; n < 4; n++)
		{
			...
			if( m_AreaInfo[i].use )	cv::line(*pImg, pt1, pt2, cv::Scalar(255, 128, 128));
			else					cv::line(*pImg, pt1, pt2, cv::Scalar(128, 128, 255));
		}
...
void CCaliper::searchCandidateEachCaliper(BYTE *pImage, int width, int height)
{
	...
	//	if( m_vtCaliperAreaInfo[i].use != TRUE )  continue;
		...
		for(int x = 0; x < m_nProjectionLength; x++)
		{
			...
			cv::Point2f*pCaliper = m_ptCaliper_LUT[0][x];
				...
				for(int y = m_nSearchLength - nPitch - 1; y > nPitch; y--)
				{
					...
					if( m_nContrastDirection == SEARCH_DIR_DARK_TO_LIGHT)		// Dark To Light
					{
						if( nValue < -m_nThreshold )
						{
							...
							{
								_curD = y;
								if (_preD == -1 || abs(_curD- _preD)> nNextLine)
								{
									nPeakCount++;
									_preD = _curD;
								}
...
void CCaliper::search_candidate_each_caliper_circle(BYTE *pImage, int width, int height)
{
	...
	for (int i = 0; i < m_nNumOfCalipers; i++)
	{
		...
		for (int y = 0; y < m_nProjectionLength; y++)
		{
			...
			cv::Point2f*pCaliper = m_ptCaliper_LUT[i][y];
			cv::Point2f*pCaliperPrev = &m_ptCaliper_LUT[i][y][-1];
			cv::Point2f*pCaliperNext = &m_ptCaliper_LUT[i][y][1];
...
{
	m_ptCaliper_LUT = new cv::Point2f **[m_nNumOfCalipers];

	for (int k = 0; k < m_nNumOfCalipers; k++)
	{
		m_ptCaliper_LUT[k] = new cv::Point2f *[m_nProjectionLength];

		for (int i = 0; i < m_nProjectionLength; i++)
			m_ptCaliper_LUT[k][i] = new cv::Point2f[m_nSearchLength];
	}
