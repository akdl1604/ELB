...
public:
	...
	cv::Mat halcon_linePreProcTest(BYTE *psrc, int w,int h,int bProc);
	...
	HTuple halcon_DispImage(cv::Mat *m_workImg,CRect rect,HWND HwndView );
	...
	Hobject MatImageToHImage(cv::Mat *pImage);
	cv::Mat HImageToMatImage(Hobject &Hobj);
	void HContourToMatImage(Hobject &Hobj,Hobject &Himage);
	...
	std::vector < std::string> m_pQRStringPtr;
