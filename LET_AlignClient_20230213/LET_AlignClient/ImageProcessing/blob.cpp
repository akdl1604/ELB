	...
	for( it;it!=en;it++)
	{
		...
		else
		{
			if( !maskImage.empty() )
			{
				...
				pMask =(maskImage.data + actualPoint.x - 1 + (actualPoint.y - 1) * maskImage.cols);
				...
				if(!find)
				{
					pMask = (maskImage.data + actualPoint.x - 1 + (actualPoint.y ) * maskImage.cols);
...
double CBlob::Mean(Mat image )
{
	...
	mask = cv::Mat::zeros(m_boundingBox.height, m_boundingBox.width, IPL_DEPTH_8U);
	...
	image(m_boundingBox).copyTo(cMat);
	cv::meanStdDev(cMat, mean, std, mask);
...
double CBlob::Max(Mat image ){
	double min, max;

	GetBoundingBox();
	cv::minMaxLoc(image(m_boundingBox), &min, &max);

	return max;
}

double CBlob::Min(Mat image ){
	double min, max;
	cv::minMaxLoc(image(m_boundingBox), &min, &max);
	return min;
}
...
void CBlob::GetConvexHull( t_contours& hull )
{
	...
	if(isJoined){
		...
		for(it=joinedBlobs.begin();it!=en;it++){
			numPts+= int((*it)->GetExternalContour()->GetContourPoints().size());
		}
...
int CBlob::overlappingPixels(CBlob *blob )
{
	...
	if(interRect.width == 0 || interRect.height==0)
		return 0;
	cv::Rect minContainingRect = r1 | r2; //Minimum containing rectangle
	...
	case PIXELWISE:
		{
			...
			drawContours(cHullMat,cHull,-1,Scalar(255),-1,8,noArray(),2147483647,cv::Point(-bbox.x,-bbox.y));
