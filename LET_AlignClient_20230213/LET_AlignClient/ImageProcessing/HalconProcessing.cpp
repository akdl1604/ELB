...
Hobject CHalconProcessing::MatImageToHImage(cv::Mat *pImage)
{
	Hobject Hobj;
	if (3 == pImage->channels())
	{
		...
		cv::split(*pImage, pImageRGB);
		gen_image3(&Hobj, "byte", pImage->cols, pImage->rows, (Hlong)(pImageRGB[0].data), (Hlong)(pImageRGB[1].data), (Hlong)(pImageRGB[2].data));
	}
	else if (1 == pImage->channels())
	{
		gen_image1(&Hobj, "byte", pImage->cols, pImage->rows, (Hlong)(pImage->data));
	}
	return Hobj;
}

cv::Mat CHalconProcessing::HImageToMatImage(Hobject &Hobj)
{
	cv::Mat pImage;
	HTuple htChannels;
	...
	if (htChannels[0].I() == 1)
	{
		...
		memcpy(pImage.data, ptr, height*width);
	}
	...
	if (htChannels[0].I() == 3)
	{
		...
		for (int i = 0; i < 3; i++)	pImageRGB[i] = cv::Mat(height, width, CV_8UC1, cv::Scalar(255));

		memcpy(pImageRGB[0].data, ptrRed, width*height);
		memcpy(pImageRGB[1].data, ptrGreen, width * height);
		memcpy(pImageRGB[2].data, ptrBlue, width * height);

		cv::merge(pImageRGB,3, pImage);

		for (int i = 0; i < 3; i++)	pImageRGB[i].release();
	}
	return pImage;
}

void CHalconProcessing::HContourToMatImage(Hobject &Hobj,Hobject &Himage)
{
	Hobject Region;
...
BOOL CHalconProcessing::halcon_ReadModel(BYTE *psrc, int w,int h, int nMinContrast, int nHighContrast)
{
	...
	get_shape_model_contours(&ho_ModelContours, hv_ModelId, 1);
//	HContourToMatImage(ho_ModelContours,ho_TemplateImage);
...
HTuple CHalconProcessing::halcon_DispImage(cv::Mat*m_workImg,CRect rect,HWND HwndView )
{
	Hobject  Image;
	...
	Image=MatImageToHImage(m_workImg);
...
cv::Mat CHalconProcessing::halcon_linePreProcTest(BYTE *psrc, int w,int h,int bProc)
{
  Hobject  ho_Angio;
  ...
  return HImageToMatImage(ho_BinImage);
}
...
int CHalconProcessing::halcon_QRcodeDetection(BYTE *psrc, int w,int h,CRect InspROI)
{
	...
	create_data_code_2d_model("Data Matrix ECC 200", "default_parameters", "maximum_recognition",  &DataCodeHandle);//QR Code
	...
	if(rVal >= 0)
	{
		for (i=0; i<=(ResultHandles.Num())-1; i+=1)
		{
			...
			data = DecodedDataStrings[i].S();
			m_pQRPosPtr.push_back(r);
			m_pQRStringPtr.push_back(data);
		}
	}
