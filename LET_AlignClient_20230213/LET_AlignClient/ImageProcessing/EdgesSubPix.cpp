...
static inline void getMagNeighbourhood(Mat &dx, Mat &dy, cv::Point &p, int w, int h, vector<double> &mag)
{
...
static inline double vector2angle(double x, double y)
{
    ...
    return a >= 0.0 ? a : a + CV_PI*2;
}

void extractSubPixPoints(Mat &dx, Mat &dy, vector<vector<cv::Point> > &contoursInPixel, vector<Contour> &contours)
{
    int w = dx.cols;
    int h = dx.rows;
...
{
    Mat blur;
    GaussianBlur(gray, blur, cv::Size(0, 0), alpha, alpha);

    Mat d;
    getCannyKernel(d, alpha);
    Mat one = Mat::ones(cv::Size(1, 1), CV_16S);
    Mat dx, dy;
