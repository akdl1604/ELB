...
void CPaneMachine::OnBnClickedBtnPatternRegistMachine()
{
	...
	m_CalibHalcon[nCam][nPos][nIndex].halcon_ReadModel(subMat.data, subMat.cols, subMat.rows, minconst, highcosnt);
...
void CPaneMachine::OnBnClickedBtnPatternViewMachine()
{
	...
	dlg.SetParam(&m_CalibHalcon[nCam][nPos][nIndex]);
	...
	if (_access(strMaskPath, 0) == 0)
	{
		...
		m_CalibHalcon[nCam][nPos][nIndex].halcon_ReadModelNew(src.data, grayimg.data, src.cols, src.rows);
	}
	m_CalibHalcon[nCam][nPos][nIndex].halcon_SetShapeModelOrigin(offsetX, offsetY);
...
BOOL CPaneMachine::findPattern_Matching(BYTE* pImage, int nCam, int nPos, int m_nWidth, int m_nHeight, CRect rcInspROI)
{
	...
	{
		if (m_CalibHalcon[nCam][nPos][nIndex].getModelRead() != true) continue;

		bRet = m_CalibHalcon[nCam][nPos][nIndex].halcon_SearchMark(pImage, m_nWidth, m_nHeight, rcInspROI, dbScoreLimit / 100.0);

		if (bRet)
		{
			score 	= m_CalibHalcon[nCam][nPos][nIndex].halcon_GetResultScore() * 100.0;
			dbPosX 	= m_CalibHalcon[nCam][nPos][nIndex].halcon_GetResultPos().x;
			dbPosY 	= m_CalibHalcon[nCam][nPos][nIndex].halcon_GetResultPos().y;
			dbAngle = m_CalibHalcon[nCam][nPos][nIndex].halcon_GetResultAngle();
