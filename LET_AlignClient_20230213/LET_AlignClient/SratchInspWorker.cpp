...
BOOL CSratchInspWorker::Inspection_ScratchLine(int nJob, int id, BYTE* ImgByte, CViewerEx* mViewer, bool bSimul)
{
	BOOL bReturn = TRUE;
	...
	if (m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getInspectionDebugCheck())
		cvHideImage("PreProcessing Image");
	...
	pFormMain->m_pDlgViewerMain[nview]->GetViewer().OnLoadImageFromPtr(imgSrc.data);
	...
	// 디버깅 모드에서 이미지 보기
	if (m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getInspectionDebugCheck())
		cvShowImage("Scratch Insp Org Image", imgSrc);

	BYTE* pImage = imgSrc.data;
	int _inspPos = 0;

	InspROI = m_pMain->GetMatching(nJob).getSearchROI(0, _inspPos);
	pDC->Rectangle(CRect(InspROI.left, InspROI.top, InspROI.right, InspROI.bottom));

	m_pMain->GetMatching(nJob).findPattern(pImage, 0, _inspPos, W, H);
	BOOL bFind = m_pMain->GetMatching(nJob).getFindInfo(0, _inspPos).GetFound() == FIND_OK ? TRUE : FALSE;

	double max_blob_width = 0.0;
	double max_blob_height = 0.0;

	cv::Mat colorProc;
	cv::cvtColor(imgSrc, colorProc, COLOR_GRAY2BGR);

	if (bFind)
	{
		// /*  중요 : 아래 roiOffset은 스펙에 맞게 입력 해야함 */
		double dLeftTopDistX = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getScratchLeftTopDistX();
		double dLeftTopDistY = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getScratchLeftTopDistY();
		double dInspWidth = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getScratchInspWidth();
		double dInspHeight = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getScratchInspHeight();
		int dThresh = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getScratchThresh();
		int roiOffsetX = int(dLeftTopDistX / xres); //0.01
		int roiOffsetY = int(dLeftTopDistY / yres);   //4.6
		int roiLengthOffsetX = int(dInspWidth / yres); //0.05
		int roiLengthOffsetY = int(dInspHeight / yres);

		double dbPosX = m_pMain->GetMatching(nJob).getFindInfo(0, _inspPos).GetXPos();
		double dbPosY = m_pMain->GetMatching(nJob).getFindInfo(0, _inspPos).GetYPos();

		cv::Rect _Roi = cv::Rect(dbPosX + roiOffsetX, dbPosY - roiOffsetY, roiLengthOffsetX, roiLengthOffsetY);
		...
		pDC->SelectObject(&penOK);
		pDC->Rectangle(CRect(_Roi.x, _Roi.y, _Roi.x+ _Roi.width, _Roi.y+ _Roi.height));

		Gdiplus::Color color = 0xffff0000;
		pDC->AddGraphic(new GraphicPoint(Gdiplus::PointF(dbPosX, dbPosY), color, 5, 2));

		cv::line(colorProc, cv::Point(dbPosX-20, dbPosY), cv::Point(dbPosX+20, dbPosY), cv::Scalar(64, 255, 64), 2);
		cv::line(colorProc, cv::Point(dbPosX, dbPosY-20), cv::Point(dbPosX, dbPosY+20), cv::Scalar(64, 255, 64), 2);

		if (_Roi.width > 0 && _Roi.height > 0)
		{
			gray = imgSrc(_Roi).clone();

			sobelDirection(3, SOBEL_LEFT, gray, gray);

			threshold(gray, iproc, dThresh, 255, CV_THRESH_BINARY);

			if (m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getInspectionDebugCheck())
			{
				cvShowImage("Enhance Bin Image", iproc);
				cvShowImage("PreProcessing", gray);
			}
			...
			m_Blob.InitValue();
			m_Blob.SetImgInfo(iproc.cols, iproc.rows, 1, iproc.cols);
			m_Blob.SetRoi(CRect(4, 4, iproc.cols - 4, iproc.rows - 4));

			m_Blob.BlobScan((LPBYTE)iproc.data);
			m_Blob.ExtractBlob();
			...
			for (int i = 1; i < nPtCnt; i++)
			{
				...
				roi.left = lpCurBlob->left + _Roi.x;
				roi.top = (iproc.rows - lpCurBlob->bottom) + _Roi.y;
				roi.right = lpCurBlob->right + _Roi.x;
				roi.bottom = (iproc.rows - lpCurBlob->top) + _Roi.y;

				pDC->Rectangle(CRect(roi.left, roi.top, roi.right, roi.bottom));

				vtBlob.push_back(m_Blob.GetBlob(i));

				if (int(lpCurBlob->pxlSize) > pxlSizeSpec && min_size < (yres * roi.Height()))
				{
					nRealDefectCount++;
			...
			if (m_pMain->vt_job_info[nJob].num_of_viewer > 1)
			{
				...
				{
					CRect roi;
					roi.left = vtBlob[i]->left + _Roi.x;
					roi.top = (iproc.rows - vtBlob[i]->bottom) + _Roi.y;
					roi.right = vtBlob[i]->right + _Roi.x;
					roi.bottom = (iproc.rows - vtBlob[i]->top) + _Roi.y;
				
					str.Format("x:%.3fmm, y:%.3fmm", fabs(roi.Width()) * xres, fabs(roi.Height()) * yres);
					pDC->TextOutA(roi.right, roi.bottom, str);

					cv::rectangle(colorProc, cv::Point(roi.left, roi.top), cv::Point(roi.right, roi.bottom), CV_RGB(173, 255, 47), 2);					
					cv::putText(colorProc, std::string(str), cv::Point(roi.right, roi.bottom), cv::FONT_ITALIC, 2, cv::Scalar(255, 64, 255), 2);
				}

				str.Format("spec : min_size = %.4fmm ,pixel count : %d", min_size,pxlSizeSpec);
				pDC->TextOutA(100, 20, str);
				cv::putText(colorProc, std::string(str), cv::Point(100,80), cv::FONT_ITALIC, 2, cv::Scalar(255, 64, 255), 5);

				if (nRealDefectCount > 0) pDC->SetTextColor(Gdiplus::Color::Blue);
				else pDC->SetTextColor(Gdiplus::Color::Green);

				str.Format("max length : %.4fmm", max_blob_height* xres);
				pDC->TextOutA(100, 80, str);	
				cv::putText(colorProc, std::string(str), cv::Point(100, 160), cv::FONT_ITALIC, 2, cv::Scalar(255, 64, 255), 5);
			}
...
#pragma region Defect 저장 
				...
				strFileDir_Image_Proc.Format("%s%s_%s_%sProcImg.jpg", strFileDir_Image, Cell_Name, Time_str, m_pMain->m_stCamInfo[ncamera].cName);
				strFileDir_Image_Raw.Format("%s%s_%s_%sImgRaw.jpg", strFileDir_Image, Cell_Name, Time_str, m_pMain->m_stCamInfo[ncamera].cName);

				if (_access(strFileDir_Image_Proc, 0) != 0) strFileDir_Image_Proc.Format("%s%s_%s_Proc_Img.jpg", strFileDir_Image, Cell_Name, real_Time_str, m_pMain->m_stCamInfo[ncamera].cName);
				if (_access(strFileDir_Image_Raw, 0) != 0)	strFileDir_Image_Raw.Format("%s%s_%s_%s_ImgRaw.jpg", strFileDir_Image, Cell_Name, real_Time_str, m_pMain->m_stCamInfo[ncamera].cName);

				cv::imwrite(std::string(strFileDir_Image_Proc), colorProc);
				cv::imwrite(std::string(strFileDir_Image_Raw), imgSrc);
...
#pragma endregion
	...
	if (bSimul != TRUE && m_pMain->m_bSimulationStart != TRUE)
	{
		...
		str.Format("Send Data Adress : %d", address);
		::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)& str);
		str.Format("max scratch width , height: %.3fmm , %.3fmm", max_blob_width, max_blob_height);
		::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)& str);
	}
