...
BOOL CPanelExistInspection::calcPanelAvgBrightness(cv::Mat& pSrcImage, int nPanel, CRect rectROI)
{
	BOOL bSuccess = FALSE;

	if (pSrcImage.empty()) return bSuccess;

	cv::Mat pCropImage = pSrcImage(cvRect(rectROI.left, rectROI.top, rectROI.Width(), rectROI.Height())).clone();
	
	cv::Scalar avg = cv::mean(pSrcImage);
	pCropImage.release();
...
/*
BOOL CPanelExistInspection::panelValidPosCheck(cv::Mat &pSrcImage)
{
	BOOL bSuccess = FALSE;
...
void CPanelExistInspection::inspectionPanelExist(cv::Mat& pSrcImage)
{
	// Panel 유무 감지 검사
	if (0)
	{
...
int CPanelExistInspection::searchSeparator(cv::Mat& pSrcImage, CRect rectROI)
{
	int nResult = 0;
	cv::Mat pImage = pSrcImage;
	...
	cv::Mat pCropImage = pImage(rtROI).clone();
	cv::GaussianBlur(pCropImage, pCropImage, cv::Size(5, 5), 1.);

	endInSearch(pCropImage, pCropImage);

	cv::Mat enhanceImg(pCropImage.rows, pCropImage.cols, CV_8UC1, pCropImage.data);
	...
	cv::threshold(pCropImage, pCropImage, 0, 255, CV_THRESH_BINARY| CV_THRESH_OTSU);

	removeNoise(enhanceImg, 100);
	dilate(pCropImage, pCropImage, NULL, cv::Point(-1, -1), 3);
	erode(pCropImage, pCropImage, NULL, cv::Point(-1, -1), 3);

	cv::Scalar scalar = cv::mean(pCropImage);
...
int CPanelExistInspection::searchSeparator2(BYTE *pImg, CRect rectROI, int nThresh)
{
	...
	cv::Mat pImage(H, W, CV_8UC1, pImg);
	...
	cv::threshold(pCropImage, pCropImage, nThresh, 255, CV_THRESH_BINARY_INV);
...
int CPanelExistInspection::searchUTG(cv::Mat& pSrcImage, CRect rectROI)
{
	int nResult = 0;
	cv::Mat pImage = pSrcImage;
	...
	cv::morphologyEx(pCropImage, pCropImage, cv::MORPH_GRADIENT, element15);
	cv::threshold(pCropImage, pCropImage, 20, 255, CV_THRESH_BINARY);

	BOOL bFind_Hori = FALSE;
	BOOL bFind_Vert = FALSE;

	int *pHori = new int[pCropImage.cols];
	int *pVert = new int[pCropImage.rows];

	memset(pHori, 0, sizeof(int) * pCropImage.cols);
	memset(pVert, 0, sizeof(int) * pCropImage.rows);

	int filterOff = 6;
	// 세로 선을 찾자.
	for (int x = filterOff; x < pCropImage.cols - filterOff; x++)
	{
		for (int y = filterOff; y < pCropImage.rows - filterOff; y++)
		{
	...
	for (int x = 0; x < pCropImage.cols; x++)
	{
		if (pHori[x] > pCropImage.cols / 2)
		{
			bFind_Vert = TRUE;
			break;
		}
	...
	for (int y = 0; y < pCropImage.rows; y++)
	{
		if (pVert[y] > pCropImage.rows / 2)
		{
			bFind_Hori = TRUE;
			break;
		}
...
int CPanelExistInspection::searchUTG2(cv::Mat& pSrcImage, CRect rectROI)
{
	int nResult = 0;
...
void CPanelExistInspection::endInSearch(cv::Mat& srcImage, cv::Mat& dstImage)
{
	int i;
...
void CPanelExistInspection::calcHistogram(cv::Mat& srcImage, int(&Histo)[256])
{
	int i, x, y;
...
void CPanelExistInspection::applyLUT(cv::Mat& srcImage, cv::Mat& dstImage, int(&LUT)[256])
{
