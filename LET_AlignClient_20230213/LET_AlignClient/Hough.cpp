...
int CHough::Transform(unsigned char* img_data, int w, int h) 
{ 
	_img_w = w; 
	_img_h = h; 
		...
		{ 
			...
			{ 
				...
				{ 
					...
					_accu[ (int)((floor(r + hough_h+0.5) * 180.0)) + t]++; 
				} 
			} 
		} 
	} 
 		...
 		{ 
 			...
 			{ 
 				...
 				{ 
 					//y = (r - x cos(t)) / sin(t) 
 					x1 = 0; 
 					y1 = int(((double)(r-(_accu_h/2)) - ((x1 - (_img_w/2) ) * cos(t * DEG2RAD))) / sin(t * DEG2RAD) + (_img_h / 2)); 
 					x2 = _img_w - 0; 
 					y2 = int(((double)(r-(_accu_h/2)) - ((x2 - (_img_w/2) ) * cos(t * DEG2RAD))) / sin(t * DEG2RAD) + (_img_h / 2)); 
 				} 
 				else 
 				{ 
 					//x = (r - y sin(t)) / cos(t); 
 					y1 = 0; 
 					x1 = int(((double)(r-(_accu_h/2)) - ((y1 - (_img_h/2) ) * sin(t * DEG2RAD))) / cos(t * DEG2RAD) + (_img_w / 2)); 
 					y2 = _img_h - 0; 
 					x2 = int(((double)(r-(_accu_h/2)) - ((y2 - (_img_h/2) ) * sin(t * DEG2RAD))) / cos(t * DEG2RAD) + (_img_w / 2)); 
 				}  
...
const unsigned int* CHough::GetAccu(int *w, int *h) 
{ 
 	*w = _accu_w; 
 	*h = _accu_h; 
