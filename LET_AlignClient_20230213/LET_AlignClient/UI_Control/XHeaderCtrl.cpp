...
CXHeaderCtrl::CXHeaderCtrl()
{
	...
	for(int i=0; i<IDX_BMP_MAX; i++)
	{
		...
		m_infoBitmap[i].hBitmap = (HBITMAP)::LoadImage(::AfxGetInstanceHandle(), (LPCTSTR)strFileName, IMAGE_BITMAP, 0, 0, 
...
CString CXHeaderCtrl::GetBitmapFileName(UINT nBitmapID)
{
	switch( nBitmapID )
	{
	case IDX_BMP_LIST_HEADER_SHADOW			:	return _T("list_header_shadow.bmp");		break;
	case IDX_BMP_LIST_HEADER_DIVISION		:	return _T("list_header_division.bmp");		break;
	}
	return _T("");
}
...
void CXHeaderCtrl::DrawCtrl(CDC* pDC)
{
	...
	for (int i = 0; i < iItems; i++)
	{
		...
		if (rectItem.right >= rectClip.left || rectItem.left <= rectClip.right)
		{
			...
			{
				//// JCW_U 2008.04.12 START
				/*SIZE size = GetBitmapSize(IDX_BMP_LIST_HEADER_DIVISION);
				SIZE size2 = GetBitmapSize(IDX_BMP_LIST_HEADER_SHADOW);
				CDC tmpDC;
				tmpDC.CreateCompatibleDC(pDC);
				tmpDC.SelectObject(GetBitmap(IDX_BMP_LIST_HEADER_DIVISION));
...
SIZE CXHeaderCtrl::GetBitmapSize(UINT nBitmapID)
{
	SIZE size;
...
HBITMAP CXHeaderCtrl::GetBitmap(UINT nBitmapID)
{
	if( nBitmapID >= IDX_BMP_MAX )
		return NULL;
...
{
	SIZE size = GetBitmapSize(IDX_BMP_LIST_HEADER_SHADOW);
	CDC tmpDC;
	tmpDC.CreateCompatibleDC(pDC);
	tmpDC.SelectObject(GetBitmap(IDX_BMP_LIST_HEADER_SHADOW));
