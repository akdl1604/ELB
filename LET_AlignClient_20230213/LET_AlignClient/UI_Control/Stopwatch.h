...
public:
   ...
   std::string show_times(const std::uint64_t times)
   {
	   std::string test_message = format_string("%d ms", times);
       return test_message;
   }
   
   std::string show_times()
   {
	   auto end_time = std::chrono::high_resolution_clock::now();
	   std::chrono::duration<double, std::milli> float_ms = end_time - start_time;

	   std::string test_message = format_string("%.4f ms", float_ms);
       return test_message;
   }
