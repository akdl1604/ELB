...
//	}
	...
	else
	{
		...
		DrawGradRect(&dc,m_rect,clLeft,clRight,m_bVertical);
	}
	...
	CFont* pFont = GetFont();
//	CString m_sTEXT;
	if( m_nWindowEx )		GetWindowText(m_sTEXT);
	else					m_sTEXT;
...
//	GetClientRect(&rect);
	...
	if(m_iAlign == 1) // center
		::DrawText(dc, m_sTEXT, -1, &m_rect, DT_SINGLELINE|DT_VCENTER|DT_CENTER);
	else if(m_iAlign == 0) // left
	{
		m_rect.left+=m_iLeftSpacing;
		::DrawText(dc, m_sTEXT, -1, &m_rect, DT_SINGLELINE|DT_VCENTER|DT_LEFT);
	}
	else //right
	{
		m_rect.right-=m_iLeftSpacing;
		::DrawText(dc, m_sTEXT, -1, &m_rect, DT_SINGLELINE|DT_VCENTER|DT_RIGHT);
	}
...
void CGradientStatic::SetWindowTextEx(LPCSTR a_lpstr)
{
	m_nWindowEx = 0;
