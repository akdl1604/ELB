...
UINT Thread_RotateCenterCalc(void* pParam)
{
	...
	while (sequence != -1)
	{
		...
		{
			auto& cam = pMain->m_Cameras[real_cam];
			if (cam)
			{
...
UINT Thread_MainViewerDisplay(void* pParam)
{
	...
	while (pView->m_bThreadMainDisplayEndFlag && pView->m_pMain->m_bProgramEnd != TRUE)
	{
		for (v = 0; v < vcount; v++) // View 수
		{
			...
			auto& cam = pView->m_pMain->m_Cameras[nCamNum];

			if (bDisplay && cam && (!cam->IsGrabbing() || cam->IsGrabContinuous()))
			{
...
void CFormMainView::fnProcess_ProcessCenterAlignInspAlgorithm(int nJob)
{
	...
	int PLC_RW_ROTATE_ANGLE             = m_pMain->vt_job_info[nJob].plc_addr_info.read_word_start + 28; // 20230130 Tkyuha 직선 도포 각도
	...
	case ALIGN_CENTER_GRAB:
	{
		...
		if (g_CommPLC.GetBit(PLC_RB_CENTER_ALIGN_REQ)) //얼라인 수행 준비 m_pMain->m_nSeqProcess[nJob] = 100;
		{
			...
			{
				for (int i = 0; i < m_pMain->vt_job_info[nJob].viewer_index.size()-1; i++)
					reset_viewerID(nJob, i);
			}
			else reset_viewer(nJob);
			SendMessageA(WM_VIEW_CONTROL, MSG_FMV_GLASSID_UPDATE, nJob);
			...
			if (method == METHOD_LINE || method == METHOD_NOTCH)
			{
				...
				g_CommPLC.GetWord(PLC_RW_ROTATE_ANGLE, 2, lData);
				m_dLineRotateAngle = MAKELONG(lData[0], lData[1]) / MOTOR_SCALE;
				...
				// YCS 2022-12-13 직선도포일 때, Inspection Length 변경하지 않는다.
				if (method == METHOD_LINE)
					totalLength = m_pMain->vt_job_info[nJob].model_info.getAlignInfo().getInspRangeLength();
		...
		else if (g_CommPLC.GetBit(PLC_RB_CENTER_CG_GRAB_REQ)) // CG Grab
		{
			...
			{
				//SJB 2022-11-19 CG Grab Exposure Time 파라미터 값으로 변경
				if (nPos > 0 && cam && cam->IsConnected())
				{
					cam->SetExposure(nPos);
					Delay(100);
				}
	...
	case ALIGN_CG_WAIT_GRAB_END:
	{
		...
		if (bGrabComplete || !g_CommPLC.GetBit(PLC_RB_CENTER_CG_GRAB_REQ))
		{
			...
			auto& cam = m_pMain->m_Cameras[real_cam];
			if (cam && cam->IsConnected()) cam->SetExposure(nExposure_value);
...
void CFormMainView::fnProcessCenterNozzleAlignAlgorithm(int nJob)
{
	...
	int PLC_RB_NOZZLE_CENTER_ALIGN_REQ = m_pMain->m_nAlignRequestRead[nJob];
	int PLC_RB_ZOZZLE_CENTER_TIP_REQ = m_pMain->m_nAlignRequestRead[nJob] + 3;

	int PLC_WB_NOZZLE_CENTER_ALIGN_JUDGE_OK = m_pMain->m_nAlignOkWrite[nJob];
	int PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_OK = m_pMain->m_nAlignOkWrite[nJob] + 13;

	int PLC_WB_NOZZLE_CENTER_ALIGN_JUDGE_NG = m_pMain->m_nAlignNgWrite[nJob];
	int PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_NG = m_pMain->m_nAlignNgWrite[nJob] + 13;
	...
	switch (nSeq[nJob])
	{
		case NOZZLE_ALIGN_CENTER_RESET:
		{
			resetProcessStart(nJob);

			g_CommPLC.SetBit(PLC_WB_NOZZLE_CENTER_ALIGN_JUDGE_OK, FALSE);
			g_CommPLC.SetBit(PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_OK, FALSE);
			g_CommPLC.SetBit(PLC_WB_NOZZLE_CENTER_ALIGN_JUDGE_NG, FALSE);
			g_CommPLC.SetBit(PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_NG, FALSE);

			CString str;
			str.Format("[%s] Process End", get_job_name(nJob));
			::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);

			nSeq[nJob] = NOZZLE_ALIGN_CENTER_GRAB;
		}
		break;
		case NOZZLE_ALIGN_CENTER_GRAB:
		{
			if (g_CommPLC.GetBit(PLC_RB_NOZZLE_CENTER_ALIGN_REQ)/* && getProcessStart(nJob) != TRUE) || m_pMain->auto_seq_test[nJob]*/)
			{
				m_pMain->auto_seq_test[nJob] = FALSE;
				...
				nSeq[nJob] = NOZZLE_ALIGN_WAIT_OFF_REQ;
			}
			else if (g_CommPLC.GetBit(PLC_RB_ZOZZLE_CENTER_TIP_REQ))
			{
				int nCam = 2; //^^;;;
				int nJob = 2; //^^;;;
				int viewer = m_pMain->vt_job_info[nJob].viewer_index[0];
				...
				m_pDlgViewerMain[viewer]->GetViewer().OnLoadImageFromPtr(m_pMain->getProcBuffer(nCam, 0));

				InspNozzleSearchAngle_Bee(m_pMain->getCameraViewBuffer(), W, H, nJob, nCam, &m_pDlgViewerMain[viewer]->GetViewer());

				if (m_pMain->NozzleTipSizeCompare(nJob))
				{
					str.Format("Nozzle Tip Size - %.3f", m_pMain->m_dNozzleTipSize * 0.9);
					::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);
	
					m_pDlgViewerMain[viewer]->GetViewer().AddSoftGraphic(new GraphicLabel(W/ 2 + 50, H / 2, str, Gdiplus::Color::Blue));
					m_pDlgViewerMain[viewer]->GetViewer().SoftDirtyRefresh();

					g_CommPLC.SetBit(PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_OK, TRUE);
				}
				else
				{
					str.Format("Nozzle Tip Size NG - %.3f", m_pMain->m_dNozzleTipSize * 0.9);
					::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);

					// hsj 2023-02-08 노즐 팁사이즈 viewer에 나타내주기
					m_pDlgViewerMain[viewer]->GetViewer().AddSoftGraphic(new GraphicLabel(W / 2 + 50, H / 2, str, Gdiplus::Color::Blue));
					m_pDlgViewerMain[viewer]->GetViewer().SoftDirtyRefresh();
					// YCS 2023-01-20 팁사이즈 NG시 알람창 
					str.Format("[%s] Nozzle Tip Size NG - %.3f", m_pMain->vt_job_info[nJob].job_name.c_str(), m_pMain->m_dNozzleTipSize * 0.9);
					::SendMessage(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PMC_ADD_PROCESS_HISTORY, (LPARAM)&str);
					CFormMainView* pView = (CFormMainView*)m_pMain->m_pForm[FORM_MAIN];
					pView->ShowErrorMessageBlink(str);

					g_CommPLC.SetBit(PLC_WB_ZOZZLE_CENTER_TIP_JUDGE_NG, TRUE);
				}

				long MaxSizeData[2] = { 0, };
				MaxSizeData[0] = LOWORD((long)(m_pMain->m_dNozzleTipSize * 0.9 * MOTOR_SCALE));
				MaxSizeData[1] = HIWORD((long)(m_pMain->m_dNozzleTipSize * 0.9 * MOTOR_SCALE));
				
				int address = m_pMain->vt_job_info[nJob].plc_addr_info.write_word_start + 12; // A1기준 D15152
				g_CommPLC.SetWord(address, 2, MaxSizeData);

				nSeq[nJob] = NOZZLE_TIP_WAIT_OFF_REQ;
			}
			break;
		case NOZZLE_ALIGN_WAIT_OFF_REQ:
		{
			if (!g_CommPLC.GetBit(PLC_RB_NOZZLE_CENTER_ALIGN_REQ))
			{
				nSeq[nJob] = NOZZLE_ALIGN_RESET_FLAG;
			}
		}
		break;
		case NOZZLE_TIP_WAIT_OFF_REQ:
		{
			if (!g_CommPLC.GetBit(PLC_RB_ZOZZLE_CENTER_TIP_REQ))
			{
				nSeq[nJob] = NOZZLE_ALIGN_RESET_FLAG;
			}
		}
		break;
		case NOZZLE_ALIGN_RESET_FLAG:
		{
			nSeq[nJob] = NOZZLE_ALIGN_CENTER_RESET;
			break;
		}
		break;
		}
...
void CFormMainView::fnProcessTeach_XYZ_StatusMeasure(int nJob)
{
	...
	if ((bit && getProcessStart(MAX_JOB-1) != TRUE))
	{
		...
		auto& cam = m_pMain->m_Cameras[nCam];
		if (cam && cam->IsConnected())
			cam->SetExposure(nExposure);
...
LRESULT CFormMainView::OnViewControl(WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_FMV_TOOL_COMMAND:
	{
...
#else
			auto& c = m_pMain->m_Cameras[cam];
			if (c) c->cvVideoOpen(fname, w, h);
#endif
		...
		case MSG_DLGVIEW_TOOL_COMMAND_STOP:
		{
			...
			if (c && c->IsRecordVideo())
			{
				Delay(300);
				AcquireSRWLockExclusive(&c->g_bufsrwlock);
				c->cvVideoClose();
				ReleaseSRWLockExclusive(&c->g_bufsrwlock);
			}			
#endif
			else m_pMain->PostMessage(WM_VIEW_CONTROL, MSG_MD_CAMERA_STOP, cam);
		}
		break;
		}
	}
...
bool CFormMainView::fnGrabSingleImage(int nJob)
{
	...
	ncamera = camBuf.at(0);
	auto& cam = m_pMain->m_Cameras[ncamera];
	if (cam && cam->IsConnected())
	{
		AcquireSRWLockExclusive(&cam->g_bufsrwlock);
	...
	else m_pMain->m_bGrabEnd[ncamera] = FALSE;
	if (cam && cam->IsConnected())
	{
...
bool CFormMainView::fnDiffWetOutInspection(int nJob, int id, CViewerEx* mViewer, BOOL m_bManual)
{
	...
	int imedianBlurFilter = (method == METHOD_LINE) ? 5 : 15;
	...
	medianBlur(s_down, image1, imedianBlurFilter);
	medianBlur(s_downorg, image2, imedianBlurFilter);
	...
	int  iDropsAfterImageRemove = int(m_pMain->vt_job_info[nJob].model_info.getAlignInfo().getDistanceInspSpecUSL(6));
	int iGaussThreshold = (method == METHOD_LINE) ? 5 : 30;
	...
	cv::threshold(diff_im, bin, iGaussThreshold, 255, CV_THRESH_BINARY); //CV_THRESH_OTSU
	...
	if (method == METHOD_CIRCLE)
	{
		...
		int nInX = 0, nInY = 0,_bmBaseX = 0,_bmBaseY = 0, nOutX = 0, nOutY = 0;
		bool bfind = false;
		...
		for (int second = 0; second < 360; second++)
		{
			...
			for (int distanceT = int(D_FindStart); distanceT < 200; distanceT++)
			{
				...
				else if (two != -1 && ((!_bFirstMetal && Raw[y * skeleton.cols + x] < 200) || (_bFirstMetal && OrgRaw[y * skeleton.cols + x] > 200)))
				{
					...
					m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[3].push_back(nOutX - _siftX);				// OUT X좌표
					m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[4].push_back(nOutY - _siftY);				// OUT Y좌표
	...
	{     
		...
		cv::Mat iproc,iprocOrg, itempProc;

#pragma region 배경 과검 제거 20230202 Tkyuha
		vector<vector<cv::Point>> v_contours;
		vector<cv::Vec4i> v_hierarchy;
		vector<cv::Point>hull;

		iprocOrg = skeleton.clone();
		erode(iprocOrg, iprocOrg, Mat::ones(17, 17, CV_8U));
		
		findContours(iprocOrg, v_contours, v_hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
		
		int v_ID = MIN(4, int(v_contours.size()));

		sort(v_contours.begin(), v_contours.end(), [](const vector<cv::Point>& c1, const vector<cv::Point>& c2) {
		...
		{
			hull.clear();
			convexHull(v_contours[i], hull, CV_CLOCKWISE, true);
			drawContours(iprocOrg, vector<vector<cv::Point>> {hull}, 0, Scalar(255, 255, 255), CV_FILLED);
	
		}

		if (v_ID != -1 && v_contours.size()>0)
		{
			bitwise_and(iprocOrg, ~binOrg, itempProc);
			bitwise_not(itempProc, binOrg);
			dilate(binOrg, binOrg, Mat::ones(5, 5, CV_8U));
		}
...
#pragma endregion
		...
		cv::resize(diff_im2, skeleton2, cv::Size(), 2, 2, CV_INTER_CUBIC);

		GaussianBlur(skeleton2, iproc, cv::Size(3, 3),0);
		...
		if (m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getInspectionDebugCheck())
		{
			...
			cvShowImage("binOrg Image", binOrg);
		...
		// 설명 추가
		// C_CALIPER_POS_2 => Glass(도포라인) 기준선
		// C_CALIPER_POS_3 => Metal Line 기준선 위
		// C_CALIPER_POS_4 => Metal Line 기준선 아래
		// C_CALIPER_POS_5 => 포켓 왼쪽 기준선
		// C_CALIPER_POS_2 => CG 찾기 기준선
		// 기본적으로 2번 3번 4번이 등록 되어 있어야 함
		...
		for (int i = 0; i < 2; i++) // Metal Line 윗라인,Metal Line 아래 라인,포켓 라인
		{
			m_pMain->m_pDlgCaliper->m_Caliper[ncamera][0][C_CALIPER_POS_3+i].processCaliper(imgSrc.data, W, H, dx, dy, dt); 
			find_line[i] = m_pMain->m_pDlgCaliper->m_Caliper[ncamera][0][C_CALIPER_POS_3+i].getIsMakeLine();
			line_info[i] = m_pMain->m_pDlgCaliper->m_Caliper[ncamera][0][C_CALIPER_POS_3+i].m_lineInfo;
		}
		...
		if (find_line[0] && find_line[1])
		{
			...
			int ID = MIN(4, int(contours.size()));

			if (ID != -1 && contours.size()>0)
			{
				_searchRoi = boundingRect(contours[0]);
				for (int i = 1; i < ID; i++)
				{
					double lw = _searchRoi.width; 
					double lh = _searchRoi.height;
					bounding_rect = boundingRect(contours[i]);
					if (bounding_rect.width > 100)
					{
						_searchRoi.x = MIN(bounding_rect.x, _searchRoi.x);
						_searchRoi.y = MIN(bounding_rect.y, _searchRoi.y);
						_searchRoi.width = MAX(bounding_rect.width, lw);
						_searchRoi.height = MAX(bounding_rect.height, lh);
					}
					else break;
				}

				//만약 찾은 Contour가 Spec 보다 크면 오인식이라고 가정함                                                                     
				if (_searchRoi.width * xres > _linespec)		m_bPreimageMode = TRUE;

				m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.x = inspRoi.x + _searchRoi.x;							//Dopo Start
				m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.y = inspRoi.x + _searchRoi.x + _searchRoi.width;		//DOpo End
			}
			...
			int _tPreData = 0,_rFalse=0; // Tkyuha 20230102 나비모양 과검 제거 목적, 20230118 추가수정
			for (int _x = _searchRoi.x; _x < _searchRoi.x + _searchRoi.width; _x += 1)
			{
				first = -1, two = -1;
				rposy = rowC, rposx = 0;
				nInX = 0, nInY = rowC;
				x = _x, _overflow = 0, _rFalse=0;// 20230118 추가수정
				...
				{  // Tkyuha 20230102 나비모양 과검 제거 목적 //아래 수정
					...
					else if (OrgRaw[_y * binOrg.cols + _x] < 100 && _rFalse<20)		_tfindRaw = true; //20230118 추가수정	
					else						_rFalse++; // 20230118 추가수정

				   if (_tfind || _tfindRaw) //
					{
						nInX = _x, nInY = (_tfindRaw && _tPreData > rowC)? _tPreData :_y;
						p_center.x = p2.x = _x, p2.y = (_tfindRaw && _tPreData > rowC) ? _tPreData : _y;
						first = GetDistance(p_center, p2);
					} //////////////////////////////////////
				}

				if (m_bTraceSearchFindLine[C_CALIPER_POS_2]) // 
					dx = m_TraceSearch_line_info[C_CALIPER_POS_2].a * _x + m_TraceSearch_line_info[C_CALIPER_POS_2].b - inspRoi.y;
				else	dx = line_info[0].a * _x + line_info[0].b - inspRoi.y;

				dy = line_info[1].a * _x + line_info[1].b - inspRoi.y;

				for (int _y = rowC; _y < rowC + _searchRoi.height; _y += 1) //wetin search
				{
					if (dy> _y || _x < 0 || _y < 0 || _x >= iproc.cols || _y >= iproc.rows) continue;
				...
				first += (first == -1) ? 0 : _wetoutOffset, nInY -= _wetoutOffset;// Wetout 보정
				two += (two==-1)?0:_ovreflowOffset, rposy += _ovreflowOffset; // Overflow 보정
...
_loopsimulation:
	...
	if (method == METHOD_LINE && m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[1].size()>0)
	{
		double distv = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getLineLengthFDC_Distance();
		int center_X = int(m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[3].size() / 2);
		int left_X = MAX(0,int(center_X - (distv / xres)));
		int right_X = MIN(int(m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[1].size()-1),int(center_X + (distv / xres)));
		int overflow_maxMinValue = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[1].at(left_X);
		int overflow_minValue = 0;
		int wetout_maxMinValue = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[0].at(left_X);
		int wetout_minValue = 0;
		...
		{
			overflow_minValue = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[1].at(i);

			// JCY 2023-01-19 Wetout, overflow Min 좌표 설정
			if (overflow_maxMinValue > overflow_minValue)
			{
				minmaxPosX[2] = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[3].at(i) + inspRoi.x;
				minmaxPosY[2] = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[4].at(i) + inspRoi.y;
				overflow_maxMinValue = overflow_minValue;
			}
				

			if (overflow_minValue * yres < m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getLineOverflowMinSpec())
			{
		...
		for (int i = left_X; i <= right_X; i++)
		{
			wetout_minValue = m_pMain->m_ELB_DiffInspResult[datasheet].m_vTraceProfile[0].at(i);

			// JCY 2023-01-19 Wetout, overflow Min 좌표 설정
			if (wetout_maxMinValue > wetout_minValue)
			{
				minmaxPosX[0] = m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[3].at(i) + inspRoi.x;
				minmaxPosY[0] = m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[4].at(i) + inspRoi.y;
				wetout_maxMinValue = wetout_minValue;
			}
				

			if (wetout_minValue * yres < m_pMain->vt_job_info[nJob].model_info.getAlignInfo().getDistanceInspSubSpec(0))
			{
				int LocX = m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[3].at(i) + inspRoi.x;
				int LocY = m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[4].at(i) + inspRoi.y;
				pDC->Ellipse(int(LocX - 1), int(LocY - 1), int(LocX + 1), int(LocY + 1));
				cv::circle(m_pMain->m_matResultImage, cv::Point(LocX, LocY), 1, cv::Scalar(255, 125, 0), 2, 5);
			}
		}

		if (overflow_maxMinValue * yres < m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getLineOverflowMinSpec())
		{
			m_pMain->m_ELB_DiffInspResult[datasheet].m_bResultSummary = FALSE;
		}

		minmax_MinValue[1] = overflow_maxMinValue * yres;
		minmax_MinValue[0] = wetout_maxMinValue * yres;
	}
	...
	if (method == METHOD_CIRCLE)
	{
		auto xX = (m_pMain->m_ELB_TraceResult.m_ELB_ResultXY.x);// W 
		auto yY = (m_pMain->m_ELB_TraceResult.m_ELB_ResultXY.y);// H 
	...
	{
		pDC->SelectObject(&penSum);
		pDC->SetTextColor(RGB(173, 255, 47));
	...
	if (method == METHOD_CIRCLE)  // Metal 영역 침범했는지 검사 -> 미도포 검사 항목으로 통합관리
	{
		...
		for (int i = 0; i < m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[0].size(); i++)
		{
			...
			{
				if (!m_pMain->vt_job_info[0].model_info.getAlignInfo().getCircle_Cink1_Cink2MergeMode())
				{
	...
	else if (method == METHOD_LINE)
	{
		...
		for (int i = 0; i < nsize; i++)
		{
			...
			//SJB 2022-11-07 Wetout, Overflow Min Max Draw 시점 변경
		//SJB 2022-11-29 Wetout, Overflow 오버레이 수정(색 통일), 사이즈 변경
		// LYS 2023-01-11 MinMax 오버레이 수정할 부분
		...
		cv::Point2f st, ed;
		ed.x = st.x = (m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.x);
		st.y = 0; ed.y = H - 1;

		mViewer->AddSoftGraphic(new GraphicLine(st, ed, 0xff00ff00));
		ed.x = st.x = (m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.y);
		mViewer->AddSoftGraphic(new GraphicLine(st, ed, 0xff00ff00));
		...
		//mViewer->AddHardGraphic(new GraphicLabel(-50.f, 110.f, fmt("[CG-Start: %.4f]mm", m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultAngle[2]), color_Lime, Anchor::XCenterMiddleBottom));
		strText.Format("[CG-Start: %.4f]mm", m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultAngle[2]);
		mViewer->AddHardGraphic(new GraphicLabel(10.f, 110.f, fmt(strText), Gdiplus::Color(64, 255, 64)));
	...
	for (int i = 0; i < m_pMain->m_ELB_DiffInspResult[datasheet + 1].m_vTraceProfile[0].size(); i++) // wetout 최대 최소 계산
	{
		...
		//SJB 2023-12-06 Wetout Min Value 저장 수정
		if (method == METHOD_LINE)
		{
			minDistance_in = minmax_MinValue[0];
...
#pragma endregion
	...
	else
	{
		if (bWetoutJudgeEnable && (minDistance_out < Spec_Out_Min || maxDistance_out > Spec_Out_MAX))
		{
			...
			{
				// KBJ 2022-10-10 검사 에러타입추가.	======================================
				m_pMain->m_ELB_DiffInspResult[datasheet]._nErrorType += ELB_Error_Type(m_pMain->m_ELB_DiffInspResult[datasheet]._nErrorType, ELB_INSP_ERROR_OVERFLOW_MIN);
				//	==========================================================================
			}
			if (maxDistance_out > Spec_Out_MAX)
			{
				// KBJ 2022-10-10 검사 에러타입추가.	======================================
				m_pMain->m_ELB_DiffInspResult[datasheet]._nErrorType += ELB_Error_Type(m_pMain->m_ELB_DiffInspResult[datasheet]._nErrorType, ELB_INSP_ERROR_OVERFLOW_MAX);
				//	==========================================================================
			}
			bReturn = false;
			str.Format("[%s] Overflow Spec - NG", m_pMain->vt_job_info[nJob].job_name.c_str());
			::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);
		}
	}
	...
	if(method == METHOD_LINE)
	{
		...
		mViewer->AddHardGraphic(new GraphicLabel(10.f, 50.f, fmt(strText), Gdiplus::Color::Pink));
	...
	if (bReturn != TRUE)
	{
		...
		for (int i = 0; i < ELB_INSP_MAX_ERROR_TYPE; i++)
		{
			if ((m_pMain->m_ELB_DiffInspResult[datasheet]._nErrorType & (1 << i)) != 0)
			{
				...
				case ELB_INSP_ERROR_OVERFLOW_MIN:	str_temp.Format("[ELB_INSP_ERROR_OVERFLOW_MIN]");	break;
				case ELB_INSP_ERROR_OVERFLOW_MAX:	str_temp.Format("[ELB_INSP_ERROR_OVERFLOW_MAX]");	break;
				}
	...
	m_pMain->m_dCG_SearchPt = cv::Point2d(0, 0);
...
BOOL CFormMainView::InspNozzleSearchInspection(BYTE* pImage, int w, int h, int nJob, int nCam, CViewerEx* mViewer, bool clr)
{
	...
	else m_pMain->m_bGrabEnd[nCam] = FALSE;
...
BOOL CFormMainView::InspNozzleSearchAngle_Bee(BYTE* pImage, int w, int h, int nJob, int nCam, CViewerEx* mViewer, BOOL Reverse)
{
	...
	const int PLC_RB_ZOZZLE_CENTER_TIP_REQ = m_pMain->m_nAlignRequestRead[nJob] + 3;
	...
	if (g_CommPLC.GetBit(PLC_RB_ZOZZLE_CENTER_TIP_REQ))	r = cv::Rect(950, 500, w - 1900, h - 700);
	else r = cv::Rect(700, 350, w - 1400, h - 700);
	double xres = m_pMain->GetMachine(nJob).getCameraResolutionX(0, 0);
	double yres = m_pMain->GetMachine(nJob).getCameraResolutionY(0, 0);
	...
	dilate(bin2, bin2, Mat(), cv::Point(-1, -1), 1 * 2);//침식, 노이즈 제거
	erode(bin2, bin2, Mat(), cv::Point(-1, -1), 1);//팽창, 밝은 영역 확장   
	...
	if (lines.size() > 0)
	{
		...
		if (ids.size() == 2)
		{
			...
			if (_noiseCount > 10)
			{
				...
				// hsj 2023-02-08 노즐 팁사이즈 viewer에 나타내주기
				mViewer->AddSoftGraphic(new GraphicLabel(w / 2 + 50, h / 2, strText, Gdiplus::Color::Blue));
...
#pragma region File Log 저장
		...
		if (writeFile.is_open())
		{
			...
			if (method == METHOD_CIRCLE)
			{
				...
				if (m_pMain->m_nCurrentCircleCount > 5)
				{
					ncc = m_pMain->m_nCurrentCircleCount / 360;
					iEtc = m_pMain->m_nCurrentCircleCount % (360 * ncc);
				}
				else
				{
					ncc = MIN(4, MAX(1, m_pMain->m_nCurrentCircleCount)); //회전수
					iEtc = 0;
				}
				...
				if (ncc != 1)
				{
					for (int j = 1; j < ncc; j++)  // 2회전 반복해서 생성
					{
						for (int i = 1; i < arrayList.size(); i++)
						{
							...
							ty = m_pMain->m_ELB_TraceResult.m_vTraceProfile[1].at(i); 
					...
					for (int i = 1; i < arrayList.size(); i++)
					{
						if (iEtc > m_pMain->m_ELB_TraceResult.m_vTraceProfile[2].at(i))
							icnt++;
						else
							break;
					}
				...
				for (int j = icnt + 1; j < nICount + icnt; j++)
				{
					tx = m_pMain->m_ELB_TraceResult.m_vTraceProfile[0].at(j);
					ty = m_pMain->m_ELB_TraceResult.m_vTraceProfile[1].at(j); 
				...
				// YCS 2022-12-29 Y 오프셋 적용 조건 수정
				for (int i = 0; i < m_pMain->m_ELB_TraceResult.m_vTraceProfile[0].size(); i++)
				{
					if (abs(m_pMain->m_dCircleFourThetaPos) != 0 && m_pMain->m_ELB_TraceResult.m_vTraceProfile[2][i] > abs(m_pMain->m_dCircleFourThetaPos))
					{
					...
					else if (abs(m_pMain->m_dCircleThirdThetaPos) != 0 && m_pMain->m_ELB_TraceResult.m_vTraceProfile[2][i] > abs(m_pMain->m_dCircleThirdThetaPos))
					{
						m_pMain->m_ELB_TraceResult.m_vTraceProfile[1][i] += m_pMain->m_nThirdCircleOffset_Y;
						//m_pMain->m_ELB_TraceResult.m_vTraceProfile[0][i] += diffSteelAlignX; // Steel 보상시 사용
						//m_pMain->m_ELB_TraceResult.m_vTraceProfile[1][i] += diffSteelAlignY;
					}
					else if (abs(m_pMain->m_dCircleSecondThetaPos) != 0 && m_pMain->m_ELB_TraceResult.m_vTraceProfile[2][i] > abs(m_pMain->m_dCircleSecondThetaPos))
					{
...
#pragma endregion
	...
	if (bNotchShift)
	{
		...
		sobelDirection(3, SOBEL_RIGHT, src, srcMask);
...
double CFormMainView::InspLineCGDetection(int nJob, int nCam, const cv::Mat* src, CViewerEx* mViewer)
{
	...
	int ncamera = camBuf.at(0);
	int datasheet = 0;
	double _searchCG_X = W / 2., _searchCG_Y=0;
	double _linecenterspec = m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getCamCenterToCG();
	...
	if (find_line[0] == TRUE)
	{
		if (pDC != NULL)
		{
			...
			_searchCG_X = (_searchPocket.x + _searchPocket.y) / 2.;
			m_pMain->m_dCG_SearchPt = cv::Point2d(_searchCG_X,H/2);

			m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultAngle[2] = _searchCG_X;	
			str.Format("[CG-LineFind] x = %.3f, y=%.3f", _searchCG_X, _searchCG_Y);
			::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);
		}
	}
	else
	{
		...
		m_pMain->GetMatching(nJob).findPattern(src_img.data, nCam, 0, W, H);
		CFindInfo pFindInfo = m_pMain->GetMatching(nJob).getFindInfo(nCam, 0);

		_searchCG_X = 0;		
		if (pFindInfo.GetFound() == FIND_OK)
		{
			_searchCG_X = m_pMain->GetMatching(nJob).getFindInfo(nCam, 0).GetXPos();
			_searchCG_Y = m_pMain->GetMatching(nJob).getFindInfo(nCam, 0).GetYPos();
			m_pMain->m_dCG_SearchPt = cv::Point2d(_searchCG_X, _searchCG_Y);
		}

		if (pDC != NULL)
		{
			if (m_pMain->m_dCG_SearchPt.x != 0)
			{
				int nRate = double(H / 2064) + double(W / 3088) - 1;
				int nPenThickness = 50*CAM_VIEW_PEN_THICKNESS * nRate;

				pDC->MoveTo(int(m_pMain->m_dCG_SearchPt.x - nPenThickness), int(m_pMain->m_dCG_SearchPt.y));
				pDC->LineTo(int(m_pMain->m_dCG_SearchPt.x + nPenThickness), int(m_pMain->m_dCG_SearchPt.y));
				pDC->MoveTo(int(m_pMain->m_dCG_SearchPt.x), int(m_pMain->m_dCG_SearchPt.y - nPenThickness));
				pDC->LineTo(int(m_pMain->m_dCG_SearchPt.x), int(m_pMain->m_dCG_SearchPt.y + nPenThickness));

				cv::Point pt1(int(m_pMain->m_dCG_SearchPt.x - nPenThickness), int(m_pMain->m_dCG_SearchPt.y));
				cv::Point pt2(int(m_pMain->m_dCG_SearchPt.x + nPenThickness), int(m_pMain->m_dCG_SearchPt.y));
				cv::line(src_img_draw, pt1, pt2, cv::Scalar(255, 0, 0), 4);
				cv::Point pt3(int(m_pMain->m_dCG_SearchPt.x), int(m_pMain->m_dCG_SearchPt.y - nPenThickness));
				cv::Point pt4(int(m_pMain->m_dCG_SearchPt.x), int(m_pMain->m_dCG_SearchPt.y + nPenThickness));
				cv::line(src_img_draw, pt3, pt4, cv::Scalar(255, 0, 0), 4);

				str.Format("[CG-MarkFind] Draw");
				::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);
			}
		...
		str.Format("[CG-MarkFind] x = %.3f, y=%.3f", _searchCG_X, _searchCG_Y);
		::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);
	}
	...
	int viewer = m_pMain->vt_job_info[nJob].viewer_index[3];
	m_pDlgViewerMain[viewer]->GetViewer().OnLoadImageFromPtr(src_img_draw.data);

	CString imgMsg, strImgDir, str_ImageType, strTime;
	SYSTEMTIME time;
	GetLocalTime(&time);
	//KMB 220924 SYSTEM OPTION에 따라 저장이미지 확장자 변경
	if (m_pMain->vt_system_option[nJob].save_image_type == FALSE)
...
BOOL CFormMainView::InspLineEdgeDetection(int nJob, const cv::Mat& src, CPoint ptStart, CPoint ptEnd, float fscanLength, CViewerEx* mViewer, bool btracking, bool bdust)
{
	...
	if (1) // if (points.size() > 10)
	{
		...
		if (m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getFeriAlignFindPreProcFlag())
		{
			cv::dilate(src, imgProc, cv::Mat());
			pImage = imgProc.data;
		}
		else pImage = src.data;
	...
	//	start = int(-fFullScanLength / 2); // 가감속 고려 안한 경우
		...
		// 얼라인 값 계산
		double alignTheata = (ptrPoint[nCount - 1].x - ptrPoint[0].x)!=0? atan((ptrPoint[nCount - 1].y - ptrPoint[0].y) * yres / ((ptrPoint[nCount - 1].x - ptrPoint[0].x) * xres)) * 180 / CV_PI:0;
		...
		if (btracking)
		{
			...
			int nAlignMethod = m_pMain->vt_job_info[nJob].model_info.getAlignInfo().getMarkFindMethod();
			if (nAlignMethod == METHOD_LINE && m_pMain->vt_job_info[nJob].model_info.getInspSpecParaInfo().getPanelToMetalCalcEnable() == TRUE)
			{
#pragma region 캘리퍼 각도 찾기 && 그리기
				sLine PN_Line; // 캘리퍼
				sLine MP_Line;
				PN_Line = m_TraceSearch_line_info[C_CALIPER_POS_2];
				MP_Line = m_TraceSearch_line_info[C_CALIPER_POS_3];

				double dCaliperLineGap = PN_Line.sy - MP_Line.sy * yres;

				CString str;
				str.Format("[% s] PN to MP Gap : %f mm", get_job_name(nJob) ,dCaliperLineGap);
				::SendMessageA(m_pMain->m_pPane[PANE_AUTO]->m_hWnd, WM_VIEW_CONTROL, MSG_PA_ADD_PROCESS_HISTORY, (LPARAM)&str);

				dCaliperLineGap = abs(PN_Line.sy - MP_Line.sy) * yres;
				long lData[2] = { 0, };
				lData[0] = LOWORD(dCaliperLineGap * 10000);
				lData[1] = HIWORD(dCaliperLineGap * 10000);
				g_CommPLC.SetWord(m_pMain->vt_job_info[nJob].plc_addr_info.write_word_start + 34, 2, lData); // A1기준 15154
...
#pragma endregion
	...
	{
		m_pMain->setAlarmMessage(ALARM_DIFF_LACK_WARNING, TRUE, "[Parameter Warning !!] - FeriAlignFindThresh");
		breturn = FALSE;
	}
...
void CFormMainView::SendFDCValue_Circle(int nJob, int real_cam)
{
	...
	// JCY 2022-12-24 Peri 도포 FDC 항목 추가 및 CG-Start, Length 텍스트 수정 										[79][80]
	if (method == METHOD_LINE)
	{
		// Wetout 1,2,3
		// Wetout Min Max
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(m_pMain->m_vInspWetOutResult[datasheet].dMin_In * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(m_pMain->m_vInspWetOutResult[datasheet].dMin_In * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(m_pMain->m_vInspWetOutResult[datasheet].dMax_In * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(m_pMain->m_vInspWetOutResult[datasheet].dMax_In * PLC_DATA_REVISION);

		// Wetout Over Min Max																					[81][82]
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(m_pMain->m_vInspWetOutResult[datasheet].dMin_Out * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(m_pMain->m_vInspWetOutResult[datasheet].dMin_Out * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(m_pMain->m_vInspWetOutResult[datasheet].dMax_Out * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(m_pMain->m_vInspWetOutResult[datasheet].dMax_Out * PLC_DATA_REVISION);
		...
		int W = m_pMain->m_stCamInfo[0].w;
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD((_linecenterspec - ((W / 2 - m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.x) * xres)) * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD((_linecenterspec - ((W / 2 - m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.x) * xres)) * PLC_DATA_REVISION);

		// Dopo Length																							[84]
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD((m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.y - m_pMain->m_ELB_DiffInspResult[0].m_ELB_ResultXY.x) * xres * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD((m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.y - m_pMain->m_ELB_DiffInspResult[0].m_ELB_ResultXY.x) * xres * PLC_DATA_REVISION);

		//Wetout, Dust true/false 																			[85][86]
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(int(m_pMain->vt_job_info[datasheet].model_info.getInspSpecParaInfo().getUseDiffInsp()) * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(int(m_pMain->vt_job_info[datasheet].model_info.getInspSpecParaInfo().getUseDiffInsp()) * PLC_DATA_REVISION);

		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = LOWORD(int(m_pMain->vt_job_info[datasheet].model_info.getInspSpecParaInfo().getUseDustInsp()) * PLC_DATA_REVISION);
		m_nFDCCount_Current++;
		pData[m_nFDCCount_Current] = HIWORD(int(m_pMain->vt_job_info[datasheet].model_info.getInspSpecParaInfo().getUseDustInsp()) * PLC_DATA_REVISION);
	}
	//
	// JCY 2022-12-29 FDC 항목 추가 액튐 유무										[87]
	// 액튐 유무
	m_nFDCCount_Current++;
	pData[m_nFDCCount_Current] = LOWORD(int(m_bLiquidDropResult * PLC_DATA_REVISION));
	m_nFDCCount_Current++;
	pData[m_nFDCCount_Current] = HIWORD(int(m_bLiquidDropResult * PLC_DATA_REVISION));
