...
UINT Thread_ViewerDisplay(void *pParam)
{
	...
	while (pView->m_bThreadViewDisplayEndFlag && pView->m_pMain->m_bProgramEnd != TRUE && pView->m_pMain->m_nCurForm == FORM_CAMERA)
	{
		auto& cam = pView->m_pMain->m_Cameras[pView->curr_cam];
		if (cam && (cam->IsGrabbing() || cam->IsGrabContinuous()))
		{
...
CFormCameraView::CFormCameraView()
	: CFormView(CFormCameraView::IDD)// , m_Tracker(this)
{
	m_nSelViewerNum = 0;
	m_pProfile = NULL;
	curr_viewer = 0;
...
void CFormCameraView::InspectionProcessing(int id)
{
	...
	case 1: //SobelLeft
	{
		sobelDirection(3, SOBEL_LEFT, sgray, iproc);
		m_ViewerCamera.OnLoadImageFromPtr((BYTE *)iproc.data);
	}
	break;
	case 2: //SobelRight
	{
		sobelDirection(3, SOBEL_RIGHT, sgray, iproc);
		m_ViewerCamera.OnLoadImageFromPtr((BYTE *)iproc.data);
	}
	break;
	case 3://SobelDown
	{
		sobelDirection(3, SOBEL_DN, sgray, iproc);
		m_ViewerCamera.OnLoadImageFromPtr((BYTE *)iproc.data);
	}
	break;
	case 4: //SobelUp
	{
		sobelDirection(3, SOBEL_UP, sgray, iproc);
		m_ViewerCamera.OnLoadImageFromPtr((BYTE *)iproc.data);
	}
	break;
	case 5: //Enhance			
