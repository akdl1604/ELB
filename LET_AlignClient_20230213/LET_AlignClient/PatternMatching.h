...
public:
	...
	void preProcessImage(int nCam, int nPos, cv::Mat& pImg);
	void filterProcessImage_hori(int nCam, int nPos, cv::Mat& pImg, cv::Mat *pMatImg, int nSobelDirection, int nThreshold);
	void filterProcessImage_vert(int nCam, int nPos, cv::Mat& pImg, cv::Mat *pMatImg, int nSobelDirection, int nThreshold);
	void InspectionEnhance(cv::Mat* src, cv::Mat* dst, int id, bool disp = true);
	void InspectionEnhance_xy7(cv::Mat *src, cv::Mat *dst);

	BOOL findHisto_Hori(int nCam, cv::Mat& pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine);
	BOOL findHisto_Vert(int nCam, cv::Mat& pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine);

	BOOL findLine_Hori(int nCam, cv::Mat& pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine);
	BOOL findLine_Vert(int nCam, cv::Mat& pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine);
