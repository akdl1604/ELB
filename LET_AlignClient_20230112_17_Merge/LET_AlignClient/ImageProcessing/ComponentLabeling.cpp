...
myCompLabeler::myCompLabeler(Mat &binImage,CBlobContour** lab,cv::Point start,cv::Point end):
	...
	for(r=startPoint.y;r<endPoint.y;r++){
		...
		if(ptrDataBinary[pos]){
			...
			//Else if so to not check for label==NULL
				...
				currentBlob = new CBlob(currentLabel,cv::Point(c,r),cv::Size(w,h));
...
void myCompLabeler::TracerInt( int startDir /*= 5*/ )
{
	...
	int sR=r,sC=c;
	int startPos = sR*w+sC;
	currentContour = new CBlobContour(cv::Point(c,r),cv::Size(w,h));
...
{	
	...
	if(numThreads>1){
		...
		for(int i=1;i<numThreads;i++){
			cv::Point offset(img.size().width,1);
