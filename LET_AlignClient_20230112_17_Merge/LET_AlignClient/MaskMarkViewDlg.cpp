...
void CMaskMarkViewDlg::SetParamInitial(int nCam, int nPos, int nIndex)
{
	...
	if (m_bUseCalibModel)
	{
		mPoc = &((CPaneMachine*)theApp.m_pFrame->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][nIndex];
...
void CMaskMarkViewDlg::GetParameterData()
{
	...
	if (m_bUseCalibModel)	mPoc = &((CPaneMachine*)theApp.m_pFrame->m_pPane[PANE_MACHINE])->m_CalibHalcon[mark_nCam][mark_nIndex];
