...
#else
			if (m_pMain->m_pBaslerCam[real_cam] || m_pMain->m_pSaperaCam[real_cam].IsOpend())
#endif
...
int TabMachineAlgoPage::seqAutoCalibration2()
{
	...
	case 1200:	// 보정 위치 이동 요청
	{
		...
		else
		{
			...
			first = m_pMain->GetMachine(m_nJobID).getPtImage(nCam, nPos).at(size);
			second = m_pMain->GetMachine(m_nJobID).getPtImage(nCam, nPos).at(size-1);
...
#ifdef _DAHUHA
		AcquireSRWLockExclusive(&m_pMain->m_pDahuhaCam[nCalibCam].g_bufsrwlock); // 동기화 시킴 Tkyuha 20211027
#else
		AcquireSRWLockExclusive(&m_pMain->m_pBaslerCam[nCalibCam]->g_bufsrwlock); // 동기화 시킴 Tkyuha 20211027
#endif
		m_pMain->m_bGrabEnd[nCalibCam] = FALSE;
#ifdef _DAHUHA
		ReleaseSRWLockExclusive(&m_pMain->m_pDahuhaCam[nCalibCam].g_bufsrwlock);
#else
		ReleaseSRWLockExclusive(&m_pMain->m_pBaslerCam[nCalibCam]->g_bufsrwlock);
#endif
#ifdef _DAHUHA
		if (m_pMain->m_pDahuhaCam[nCalibCam].isconnected())
#else
		if (m_pMain->m_pBaslerCam[nCalibCam] || m_pMain->m_pSaperaCam[nCalibCam].IsOpend())
#endif
...
void TabMachineAlgoPage::dispPatternIndex()
{
	...
	for (int i = 0; i < MAX_PATTERN_INDEX; i++)
	{
		...
		if (CFile::GetStatus(strFilePath, fs) && ((CPaneMachine*)m_pMain->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][i].getModelRead()==false)
		{
			...
			if (CFile::GetStatus(strMaskPath, fs))
				((CPaneMachine*)m_pMain->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][i].halcon_ReadModelNew(strFilePath, strMaskPath);
			else ((CPaneMachine*)m_pMain->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][i].halcon_ReadModel(strFilePath, -1, -1);
		}

		if (((CPaneMachine*)m_pMain->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][i].getModelRead())			str.Format("%d : OK", i + 1);
...
void TabMachineAlgoPage::OnBnClickedBtnPatternDelete()
{
	...
	{
		((CPaneMachine*)m_pMain->m_pPane[PANE_MACHINE])->m_CalibHalcon[nCam][nIndex].setModelRead(false);
...
void TabMachineAlgoPage::seqCalcCurrentAngle()
{
	...
	if (m_pMain->fnSetMessage(2, "Angle calculation done !") != TRUE)	return;
}
