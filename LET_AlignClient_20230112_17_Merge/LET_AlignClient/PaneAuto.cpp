...
void CPaneAuto::update_grid_inspection_spec_viewer()
{
	...
	double itemData = 0, specData = 0, specData2 = 0;
	...
	//B MAX
	_specDataV[7] = m_pMain->vt_job_info[0].model_info.getInspSpecParaInfo().getWetoutBMaxSpec();
	//R MIN
	//_specDataV[6] = m_pMain->vt_job_info[0].model_info.getInspSpecParaInfo().getWetoutRMinSpec();
	//R MAX
	//_specDataV[7] = m_pMain->vt_job_info[0].model_info.getInspSpecParaInfo().getWetoutRMaxSpec();
	...
	// YCS 2022-12-01 김진용프로 요청사항 : CINK2일 때(Metal Trace First가 참), Wetout Judge Disable이라도 무조건 WetOut NG 발생시키도록
	bool   bMetalTraceFirst = m_pMain->vt_job_info[0].model_info.getAlignInfo().getUseMetalTraceFirst();

	int row = 2;// m_pMain->m_nSpecRowCount;
	BOOL bJudge = TRUE;
	...
	for (int col = 0; col < grid->GetColumnCount(); col++)
	{
		...
		double distance = (m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.y - m_pMain->m_ELB_DiffInspResult[datasheet].m_ELB_ResultXY.x) * yres;

		if (method == METHOD_CIRCLE)  distance = m_pMain->m_ELB_DiffInspResult[datasheet].m_nLackOfInkAngleCount_Judge;

		switch (col)
		{
		case 0:		Item.strText.Format(_T("S%d"),	m_pMain->m_nSpecRowCount - 1);												break;
		case 1:		Item.strText.Format(_T("%s"), cellId);																		break;
		case 2:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_vELB_ReverseRotateCenter.x);     break;
		case 3:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_vELB_ReverseRotateCenter.y);     break;
		case 4:    
		{
		...
		//case 5:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_BminResult);						break;
		//case 6:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_RminResult);						break;
		//case 7:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_RmaxResult);						break;
		...
		break;
		case 7:		Item.strText.Format(_T("%.4f"), m_pMain->m_ELB_DiffInspResult[datasheet].m_BmaxResult);						break;
		}
		...
		else if (col == 1)
		{
			...
			if (m_pMain->m_ELB_DiffInspResult[0].m_bResultSummary == FALSE || m_pMain->m_ELB_DiffInspResult[1].m_bResultSummary == FALSE ||
				m_pMain->m_ELB_DiffInspResult[0].bType == FALSE)
		...
		else if (col < 8)  // 색표시
		{
			...
			specData = _specDataV[col];//atof(grid->GetItemText(1, col));
			//KJH 2022-05-25 Length -> BMinSpec으로 UI 변경
			//KJH 2022-06-06 R Display제거 / Length는 Line에서 사용함
			//if (col == 2 || col == 5 || col == 6) // Min값 비교
			if (col == 2 || col == 6) // Min값 비교
			{
				//specData2 = _specDataV[col+1]; //atof(grid->GetItemText(1, col + 1));
				if (itemData < specData) // || itemData > specData2)
				{
					if(col == 6 && !bWetoutBJudgeModeEnable)
					{
						grid->SetItemBkColour(row, col, COLOR_ORANGE);
					}					
					else if(col == 2 && !bWetoutMinMaxJudgeModeEnable)
					{
						grid->SetItemBkColour(row, col, COLOR_ORANGE);
					}					
					else if (col == 6 && method == METHOD_LINE)	//SJB 2022-11-11 Overflow Min Grid 수정
					{
			...
			else if(col == 3 || col == 7) // Max값 비교
			{
				//specData2 = _specDataV[col-1]; //atof(grid->GetItemText(1, col - 1));
				if (itemData > specData)	// || itemData < specData2)
				{

					if (col == 7 && !bWetoutBJudgeModeEnable)
					{
						grid->SetItemBkColour(row, col, COLOR_ORANGE);
					}
					else if (col == 3 && !bWetoutMinMaxJudgeModeEnable)
					{
						grid->SetItemBkColour(row, col, COLOR_ORANGE);
					}
					else
			...
			else if (col == 4 && method == METHOD_CIRCLE) // Max Out값 비교
			{
				if (itemData > specData) // || itemData < specData2)
				{
			...
			else if (col == 4 && method == METHOD_LINE)
			{
				if (itemData > specData) // || itemData < specData2)
				{
				...
				//specData2 = m_pMain->vt_job_info[0].model_info.getAlignInfo().getDistanceInspSubSpec(1);
				if (itemData > specData) // || itemData < specData2)
				{
					if (!bWetoutMinMaxJudgeModeEnable)
						grid->SetItemBkColour(row, col, COLOR_ORANGE);
			...
			else if (col == 5 && method == METHOD_CIRCLE)
			{
				if (itemData > specData) grid->SetItemBkColour(row, col, COLOR_RED);
				else grid->SetItemBkColour(row, col, COLOR_GREEN);
			}
			//SJB 2022-11-11 Overflow Min Grid 수정
			else if (col == 5 && method == METHOD_LINE)
			{
				if (itemData < specData) grid->SetItemBkColour(row, col, COLOR_RED);
				else grid->SetItemBkColour(row, col, COLOR_GREEN);
			}
...
void CPaneAuto::update_grid_inspection_spec()
{
	...
	for (int col = 1; col < grid->GetColumnCount(); col++)
	{
		...
		break;
		case 7:		Item.strText.Format(_T("%.4f"), B_Max);																						break;
		...
		case 8:	    Item.strText.Format(_T("Comment"));																							break;
		}
	...
	for (int row = 0; row < t_row; row++)
	{
		for (int col = 0; col < t_col; col++)
		{
			...
			if (row == 0)
			{
				...
				{
					//KJH 2022-06-06 R Display제거 / Length는 Line에서 사용함
					Item.strText.Format(_T("Max(B)"));
					UINT state = grid.GetItemState(row, col);
					grid.SetItemFgColour(0, col, COLOR_BLACK);	///SJB 2022-11-30 Result Grid 0row 항목 텍스트 색 변경
					grid.SetItemState(row, col, state | GVIS_FILTER | GVIS_EDITABLE | GVIS_READONLY);
					grid.SetItemBkColour(0, 7, COLOR_PINK);
				}
				break;
				case 8:
				{
