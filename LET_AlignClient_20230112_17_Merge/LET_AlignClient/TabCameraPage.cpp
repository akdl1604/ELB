...
#ifdef _DAHUHA
	if (m_pMain->m_pDahuhaCam[real_cam].isconnected() && m_pMain->m_pDahuhaCam[real_cam].isgrabbing())
#else
...
void TabCameraPage::OnBnClickedBtnImageOpen()
{
	...
	else
	{
		...
		if (pImage->width != camW || pImage->height != camH || (pImage->nChannels*8)!=bpp)
		{
			((CFormCameraView*)m_pMain->m_pForm[FORM_CAMERA])->m_ViewerCamera.OnInitWithCamera(pImage->width, pImage->height, 8);			
		}
		...
		((CFormCameraView*)m_pMain->m_pForm[FORM_CAMERA])->m_ViewerCamera.OnLoadImageFromPtr(pImage->imageData);		
	...
	if (pScrollBar)
	{
		...
		if (pScrollBar == (CScrollBar*)&m_sliderExposureTime)
		{
			...
			if (m_pMain->m_pDahuhaCam[real_cam].isconnected())
				m_pMain->m_pDahuhaCam[real_cam].setExposureTime(double(nPos));
#else
			if(m_pMain->m_pBaslerCam[real_cam] != NULL) m_pMain->m_pBaslerCam[real_cam]->SetExposure(nPos);
#endif
...
void TabCameraPage::OnBnClickedBtnSetSubMark()
{
	...
	strTemp.Format("offX: %.2f offY: %.2f", dx, dy);
...
void TabCameraPage::dispMarkOffset()
{
	...
	strTemp.Format("%.2f", m_pMain->GetMatching(nJob).getMarkOffsetX(nCam, nPos, nIndex));
	m_lblMarkOffsetX.SetText(strTemp);

	strTemp.Format("%.2f", m_pMain->GetMatching(nJob).getMarkOffsetY(nCam, nPos, nIndex));
...
#ifdef _DAHUHA
	if (m_pMain->m_pDahuhaCam[real_cam].isconnected())
		m_pMain->m_pDahuhaCam[real_cam].setExposureTime(double(v));
#else
