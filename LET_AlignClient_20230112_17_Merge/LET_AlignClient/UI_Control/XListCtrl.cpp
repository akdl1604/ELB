...
void CXListCtrl::ShowComboBox(int nItem, int nSubItem)
{
	...
	pComboBox->SetDropDownHeight(pComboBox, 50);
		...
		if( nSubItem != -1 )
		{
			...
			if( pXLCD[nSubItem].nCheckedState >= 0 )
			{
				...
				if( pWnd && ::IsWindow(pWnd->m_hWnd) )
				{
					...
					NMI.sText = _T("");
					nResult = int(pWnd->SendMessage(WM_XLISTCTRL_CHECKBOX_CLICKING, (WPARAM)&NMI, (LPARAM)nChecked));
				}
...
void CXListCtrl::Sort(int nSubItem, BOOL bSort)
{
    ...
    for(int i=0; i<nCount; i++) 
    {
		...
		CSortData* pItem = new CSortData(str, dw);
		SetItemData( i, (DWORD_PTR)pItem);
    }
