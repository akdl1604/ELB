...
void CChartLegend::SetFontSize(int iPointSize)
{
	m_iFontSize = iPointSize;
}
...
void CChartLegend::Draw(CDC *pDC)
{
	...
	while (CChartSerie* pSerie=m_pParentCtrl->GetNextSerie())
	{
		...
		if (!m_bIsHorizontal)		rectBitmap.OffsetRect(0,MaxHeight+2);
		else			rectBitmap.OffsetRect(m_BitmapSize.cx+4+TextSize.cx+10,0);
	}
