...
{
	BOOL bRet = LoadIniFile((CString)lpFileName, 2);
	if(!bRet) return (UINT)-1;
	if(lpDefault == NULL) return -1;
	if(m_dwUseSize == 0) return -1;
...
{
	int posRet = 0;
	int secLen = int(::strlen(lpAppName));
	LPSTR pHead = (LPSTR)m_pTrueData;
...
{
	int posRet = 0;
	int secLen = int(::wcslen(lpAppName));
	LPWSTR pHead = (LPWSTR)m_pTrueData;
    ...
    )
{
	...
	{
		int len = int(strlen(lpAppName) + strlen(lpKeyName) + strlen(lpString));
		len += 7;
...
//		int len = ::wcslen(buffer);
		...
		int len = int(strlen(lpKeyName) + strlen(lpString));
		len += 3;
		buffer = new char[len+1];
		::sprintf(buffer, "%s=%s\r\n", lpKeyName, lpString);
		buffer[len] = 0;
...
{
	BOOL bRet = this->LoadIniFile((CString)lpFileName, 2);
	if(!bRet) return FALSE;
	m_bDirty = TRUE;
    ...
    if(uKey == (UINT)-1 && uSec == (UINT)-1) //no section
	{
		int len = int(wcslen(lpAppName) + wcslen(lpKeyName) +wcslen(lpString));
		len += 7;
...
//		int len = ::wcslen(buffer);
		...
		int len = int(wcslen(lpKeyName) + wcslen(lpString));
		len += 3;
		buffer = new WCHAR[len+1];
		::swprintf(buffer, L"%s=%s\r\n", lpKeyName, lpString);
		buffer[len] = 0;
						...
						LPCSTR szSection, LPCSTR szKey, UINT& posSection)
{
	...
	LPSTR p1 = pSecHead;
	LPSTR p2 = pSecHead;
	int lenSection = int(::strlen(szSection));
						...
						LPCWSTR szSection, LPCWSTR szKey, UINT& posSection)
{
	...
	LPWSTR p1 = pSecHead;
	LPWSTR p2 = pSecHead;
	int lenSection = int(::wcslen(szSection));
...
{
    int lenTargetSec =int( wcslen(lpAppName));
	...
	if(p1- pHead == (int)(m_dwUseSize/sizeof(WCHAR)) - 1)
	{
        ...
        m_dwUseSize = int(sizeof(WCHAR)*(pSecHead-pHead));
		return 0;
	}
	else
	{
		int memLen = int(m_dwUseSize - (p1 - pHead)*sizeof(WCHAR));
		//C++ Run-time Lib handle overlapped mem
        memmove(pSecHead, p1, memLen);
		m_dwUseSize = int( m_dwUseSize - (sizeof(WCHAR)*(p1 - pSecHead)));
	}
	return 0;
}
    ...
    )
{
	...
	else //key exist
	{
	    ...
	    while(*pValue == WCHAR('\r') || *pValue == WCHAR('\n'))
			pValue++;
		int lenKey = int(pValue - pHead - uKey);
        LPWSTR pKeyHead = pHead + uKey;
		int lenMove = int( m_dwUseSize - (pValue - pHead)*sizeof(WCHAR));
    ...
    )
{
	...
	if(lpNextSec == NULL)
	{
		ASSERT(distance == -1);
		//This is the Last Section
        m_dwUseSize =  int(sizeof(WCHAR)*(lpPos-lpHead));
		return 0;
	}
	else
	{
		int memLen = int( m_dwUseSize - (lpNextSec - lpHead)*sizeof(WCHAR));
		//C++ Run-time Lib handle overlapped mem
        memmove((LPVOID)lpPos, (LPVOID)lpNextSec, memLen);
		m_dwUseSize -=  int(sizeof(WCHAR)*(lpNextSec - lpPos));
	}
    
	//m_bSectionMapDirty = TRUE;
	//return TRUE;

	int lenSec =  int(lpNextSec - lpPos);
	if(m_bSectionMapLock)
	{ 
		m_bSectionMapDirty = TRUE;
	}
...
{
	BOOL bRet = LoadIniFile((CString)lpFileName, 2);  //Unicode Text
	if(!bRet) 
	{
	...
	int oldLen =  int(::wcslen(lpAppName));
	int newLen =  int(::wcslen(lpNewName));
	...
	if(newLen < oldLen)
	{
		int memLen =  int(m_dwUseSize - (lpSection - pHead + oldLen + 2)*sizeof(WCHAR));
		//C++ Run-time Lib handle overlapped mem
 ...
 )
{
    ...
    secBodySize -=  int(2 + ::wcslen(lpAppName));
		...
		(secBodySize+::wcslen(lpNewName))*sizeof(WCHAR)) > JXFlushIncrement - 5*JXPage)
	{
        ...
        ::swprintf(buffer, L"[%s]", lpNewName);
		DWORD dwRet;
		::WriteFile(hFile, buffer,  DWORD(sizeof(WCHAR)*(2 + ::wcslen(lpNewName))), &dwRet, NULL);
	...
	else
	{
        ...
        m_dwUseSize +=  DWORD((secBodySize + ::wcslen(lpNewName) + 2)*sizeof(WCHAR)); 			
	}
	return TRUE;
}
       ...
       LPCWSTR lpFileName)        // initialization file name
{
	...
	if(lpNextSec == NULL)
	{
		ASSERT(distance == -1);
		//This is the Last Section
        return int(m_dwUseSize/sizeof(WCHAR) - (lpPos-pHead));
	}
	else
	{
        return int(lpNextSec - lpPos);
	}
    return 0;
}
