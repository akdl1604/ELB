...
DWORD TCP::cSocket::Listen(DWORD u32_BindIP, USHORT u16_Port, DWORD u32_EventTimeout, DWORD u32_MaxIdleTime)
{
	...
	DWORD         dwBytesRet=0;   
	if (WSAIoctl(pk_Data->h_Socket, SIO_KEEPALIVE_VALS, &alive, sizeof(alive), NULL, 0, &dwBytesRet, NULL, NULL) == SOCKET_ERROR)   
	{   
		return WSAGetLastError();   
	}   
...
DWORD TCP::cSocket::ConnectTo(DWORD u32_ServIP, USHORT u16_Port, DWORD u32_EventTimeout, DWORD u32_MaxIdleTime)
{
	...
	// 주소가 127.0.0.1인 경우에만 들어 오도록......
	if( u32_ServIP == htonl(127 << 24 | 1) )
	{
		struct tcp_keepalive   alive;   
		alive.onoff = TRUE;   
		alive.keepalivetime = 5000;   
	...
	//setsockopt( pk_Data->h_Socket , SOL_SOCKET , SO_UPDATE_ACCEPT_CONTEXT ,(const char*) &socketValue    ,sizeof(SOCKET));  
...
{
	s8_Buf[0] = 0;
 	if (u32_Events & FD_ACCEPT)                   
		strcat(s8_Buf, "FD_ACCEPT ");
	if (u32_Events & FD_CONNECT)                  
		strcat(s8_Buf, "FD_CONNECT ");
	if (u32_Events & FD_CLOSE)                    
		strcat(s8_Buf, "FD_CLOSE ");
	if (u32_Events & FD_TIMEOUT)                  
		strcat(s8_Buf, "FD_TIMEOUT ");
	if (u32_Events & FD_READ)                     
		strcat(s8_Buf, "FD_READ ");
	if (u32_Events & FD_WRITE)                    
		strcat(s8_Buf, "FD_WRITE ");
