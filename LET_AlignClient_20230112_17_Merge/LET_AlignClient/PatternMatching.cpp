...
BOOL CPatternMatching::findPatternEdgeLine(BYTE *pImage, CRect rcInspROI, int nCam, int nPos, int m_nWidth, int m_nHeight, CFindInfo *pFindInfo)
{
	...
	IplImage *pOrgImage = cvCreateImage(cvSize(m_nWidth, m_nHeight), 8, 1);
	IplImage *pCropImage = cvCreateImage(cvSize(rect.width, rect.height), 8, 1);

	memcpy(pOrgImage->imageData, pImage, m_nWidth * m_nHeight);
	cvSetImageROI(pOrgImage, cvRect(rect.x, rect.y, rect.width, rect.height));
	cvCopy(pOrgImage, pCropImage);

	// 중간 필터
	cvSmooth(pCropImage, pCropImage, CV_MEDIAN, 5, 5);
	...
	cvResetImageROI(pOrgImage);

	int nSobelThreshold = 200;
	int nRemoveNoiseSize = 20;

	cvThreshold(pCropImage, pCropImage, nSobelThreshold, 255, CV_THRESH_BINARY);
	...
	if (nPos == 0 || nPos == 2)
	{
		for (int y = rect.height - 5; y > 0; y--)
		{
			...
			{
				if ((UCHAR)pCropImage->imageData[y * pCropImage->widthStep + x] == 255)			pVert[y]++;
			}
		...
		for (int y = 0; y < rect.height; y++)
		{
			if (y > (nMaxY - nRange) && y < (nMaxY + nRange))
				memset(&pCropImage->imageData[y * pCropImage->widthStep], 0, pCropImage->widthStep);
		}
	...
	else
	{
		...
		for (int x = 0; x < rect.width; x++)
		{
			...
			{
				for (int y = 0; y < rect.height; y++)
					pCropImage->imageData[y * pCropImage->widthStep + x] = 0;
			}
		}
...
void CPatternMatching::calcHistogram(IplImage *srcImage, int(&Histo)[256])
{
	int i, x, y;
...
void CPatternMatching::applyLUT(IplImage * srcImage, IplImage * dstImage, int(&LUT)[256])
{
...
void CPatternMatching::endInSearch(IplImage * srcImage, IplImage * dstImage)
{
	int i;
...
void CPatternMatching::removeNoise(IplImage *pImage, int nSize)
{
	// noise 제거
...
void CPatternMatching::sobelDirection(int nMaskSize, int nDirection, IplImage *srcImage, IplImage *dstImage)
{
	CvMat mtSobel;
	float *mask;
	...
	int gauss, bool white)
{
	...
	//bottom라인 찾기
	{
		...
		memcpy(pOrgImage->imageData, pImage, w * h);
		cvSetImageROI(pOrgImage, cvRect(rect.x, rect.y, rect.width, rect.height));
		cvCopy(pOrgImage, pCropImage);

		// 중간 필터
		cvSmooth(pCropImage, pCropImage, CV_MEDIAN, gauss, gauss);
		...
		// 이진화l
		cvThreshold(pCropImage, pCropImage, nSobelTheshold, 255, CV_THRESH_BINARY);
...
{
	IplImage *pImage = cvCreateImage(cvSize(width, height), 8, 1);
	memcpy(pImage->imageData, pOrgImage, width * height);
	IplImage *pVertImage, *pHoriImage;
	...
	pHoriImage = cvCreateImage(cvSize(nSize, nHoriSearchArea), 8, 1);
	cvSetImageROI(pImage, cvRectROI);
	cvCopy(pImage, pHoriImage);
	cvResetImageROI(pImage);

	// 전처리
	preProcessImage(nCam, nPos, pHoriImage);

	cv::Mat sgray(pHoriImage->height, pHoriImage->width, CV_8UC1, pHoriImage->imageData);
	...
	nSize = nSize / 4 * 4;
	int nbottomSize = pImage->height - rcROI.bottom > 0 ? pImage->height - rcROI.bottom : 100;

	// 세로성분 찾기	: X 좌표
	switch (nCornerType) {
	case CORNER_LB:			// 1사분면
		cvRectROI.x = rcROI.left + nVertSearchArea < pImage->width - 1 ? rcROI.left : pImage->width - 1 - nVertSearchArea;
		cvRectROI.y = rcROI.top;
		break;
	case CORNER_RB:			// 2사분면
	...
	case CORNER_LT:			// 4사분면
		cvRectROI.x = rcROI.left + nVertSearchArea < pImage->width - 1 ? rcROI.left : pImage->width - 1 - nVertSearchArea;
		cvRectROI.y = pImage->height - nbottomSize - nSize;//pImage->height - 1 - nSize;
		break;
	}
	...
	pVertImage = cvCreateImage(cvSize(nVertSearchArea, nSize), 8, 1);
	cvSetImageROI(pImage, cvRectROI);
	cvCopy(pImage, pVertImage);
	cvResetImageROI(pImage);

	// 전처리
	preProcessImage(nCam, nPos, pVertImage);

	cv::Mat sgray2(pVertImage->height, pVertImage->width, CV_8UC1, pVertImage->imageData);
...
{
	IplImage *pSrcImage = cvCreateImage(cvSize(width, height), 8, 1);
	memcpy(pSrcImage->imageData, pImage, width * height);
	...
	IplImage *pCropImage = cvCreateImage(cvSize(rcROI.width, rcROI.height), 8, 1);

	cvSetImageROI(pSrcImage, rcROI);
	cvCopy(pSrcImage, pCropImage);
	cvResetImageROI(pSrcImage);
	cvSmooth(pCropImage, pCropImage, CV_GAUSSIAN, 3, 3);
	...
	cvThreshold(pCropImage, pCropImage, nThreshold, 255, CV_THRESH_BINARY);
	removeNoise(pCropImage, nRemoveNoiseSize);
	...
	case 0:			// Left
	{
		...
		for (int y = 1; y < rcROI.height - 1; y++)
		{
			...
			{
				if (x >= pCropImage->width) continue;
				if ((UCHAR)pCropImage->imageData[y * pCropImage->widthStep + x] == 255)
				{
	...
	case 2:
	{
		...
		for (int x = 0; x < rcROI.width; x++)
		{
			for (int y = 1; y < rcROI.height - 1; y++)
			{
				if (y >= pCropImage->height) continue;
				if ((UCHAR)pCropImage->imageData[y * pCropImage->widthStep + x] == 255)
				{
					pContour[nValidCount].x = x + rcROI.x;
					pContour[nValidCount].y = y + rcROI.y;
...
BOOL CPatternMatching::findMoldingEdgeByRansac(BYTE *pImage, int width, int height, int nDirection, int nSearchRange, int nRemoveNoiseSize, int nThreshold, cv::Rect rectROI, sLine *pLine)
{
	...
	cvSetImageROI(pSrcImage, rectROI);
	cvCopy(pSrcImage, pCropImage);
	cvResetImageROI(pSrcImage);
	cvSmooth(pCropImage, pCropImage, CV_GAUSSIAN, 3, 3);
...
void CPatternMatching::preProcessImage(int nCam, int nPos, IplImage *pImg)
{
	switch (m_nPreProcess[nCam][nPos]) {
	case PRE_NONE:	break;
	case PRE_GAUSSIAN_3:	cvSmooth(pImg, pImg, CV_GAUSSIAN, 3, 3);	break;
	case PRE_MEDIAN_3:	cvSmooth(pImg, pImg, CV_MEDIAN, 3, 3);			break;
	case PRE_MEDIAN_5:	cvSmooth(pImg, pImg, CV_MEDIAN, 5, 5);			break;
	case PRE_MEDIAN_7:	cvSmooth(pImg, pImg, CV_MEDIAN, 7, 7);			break;
	case PRE_MEDIAN_11:	cvSmooth(pImg, pImg, CV_MEDIAN, 11, 11);		break;
	case PRE_CLOSE_1:	cvErode(pImg, pImg, NULL, 1);		cvDilate(pImg, pImg, NULL, 1);	break;
	case PRE_CLOSE_3:	cvErode(pImg, pImg, NULL, 3);		cvDilate(pImg, pImg, NULL, 3);	break;
	case PRE_CLOSE_5:	cvErode(pImg, pImg, NULL, 3);		cvDilate(pImg, pImg, NULL, 5);	break;
	}
}

BOOL CPatternMatching::findLine_Vert(int nCam, IplImage *pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine)
{
	BOOL bFind = TRUE;
	...
	if (nCornerType == CORNER_LB || nCornerType == CORNER_LT)
	{
		if (bInnerToOuter)
		{
			for (int x = nSearchArea - 1; x > 0; x--)
			{
				...
				{
					if ((UCHAR)pImg->imageData[y * pImg->widthStep + x] == 255)
					{
						pHisto[x]++;
...
BOOL CPatternMatching::findLine_Hori(int nCam, IplImage *pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine)
{
	BOOL bFind = TRUE;
	BOOL bFindY = FALSE;
	int nThresh = nSize / 4;
...
BOOL CPatternMatching::findHisto_Hori(int nCam, IplImage *pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine)
{
	BOOL bFind = FALSE;
...
BOOL CPatternMatching::findHisto_Vert(int nCam, IplImage *pImg, int nSize, int nSearchArea, int nCornerType, cv::Rect cvRectROI, BOOL bInnerToOuter, sLine *pLine)
{
	BOOL bFind = FALSE;
...
void CPatternMatching::filterProcessImage_hori(int nCam, int nPos, IplImage *pImg, cv::Mat *pMatImg, int nSobelDirection, int nThreshold)
{
	...
	case EDGE_ENHANCE_B:
	{
		...
		if (nTemp < 0) nTemp = 1;
		cvThreshold(pImg, pImg, nTemp, 255, CV_THRESH_BINARY_INV);
	}
	break;
	case EDGE_ENHANCE_W_V2:
	{
		...
		cvThreshold(pImg, pImg, nTemp, 255, CV_THRESH_BINARY);
	}
	break;
	case EDGE_ENHANCE_W:
	{
...
void CPatternMatching::filterProcessImage_vert(int nCam, int nPos, IplImage *pImg, cv::Mat *pMatImg, int nSobelDirection, int nThreshold)
{
	switch (m_nEdgeProcess[nCam][nPos]) {
	case EDGE_SOBEL:
	{
